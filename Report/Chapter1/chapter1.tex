...
Sự phát triển của học sâu đã tạo ra nhiều phương pháp hỗ trợ tự động hóa quá trình này. Các khảo sát tổng quan cho thấy phân đoạn ảnh y khoa là một trong những ứng dụng thành công nhất của học sâu hiện đại~\cite{litjens2017survey,shen2017review,minaee2021survey}. Từ các kiến trúc phân đoạn tổng quát như FCN~\cite{long2015fcn}, SegNet~\cite{badrinarayanan2017segnet} và DeepLabv3+~\cite{chen2018deeplabv3plus}, cộng đồng nghiên cứu đã phát triển các mô hình chuyên biệt hơn cho dữ liệu y khoa như V-Net~\cite{milletari2016vnet}, U-Net~\cite{ronneberger2015unet}, UNet++~\cite{zhou2018unetplusplus} và nnU-Net~\cite{isensee2021nnunet}. Gần đây, Segment Anything Model (SAM)~\cite{2023-SAM-Kirillov} và các mô hình thích nghi cho ảnh y khoa như SAM-Med2D~\cite{2023-SAMMed2D-Cheng} mở rộng hướng phân đoạn dựa trên câu nhắc trực quan, trong đó người dùng cung cấp điểm, hộp giới hạn để xác định vùng cần xử lý.

Phân đoạn dựa trên câu nhắc tạo ra một hướng kết hợp giữa khả năng định vị của con người và khả năng phân đoạn ở cấp độ điểm ảnh của mô hình. Thay vì yêu cầu mô hình tự động tìm mọi vùng bất thường trên toàn ảnh, người dùng có thể cung cấp một hộp giới hạn sơ bộ, còn mô hình thực hiện phần khoanh đường biên chi tiết. Hướng tiếp cận này cũng phù hợp với xu thế phát triển các hệ thống có sự tham gia trực tiếp của người dùng trong ảnh y khoa, nơi mô hình được xem là công cụ hỗ trợ thao tác chuyên môn thay vì thay thế hoàn toàn người dùng~\cite{xu2016deepinteractive,wang2019deepigeos,ma2024medsam}.
...
Các mô hình nền tảng Transformer tuy linh hoạt nhưng đòi hỏi tài nguyên tính toán lớn, gây khó khăn khi triển khai trên phần cứng y tế và làm chậm thời gian phản hồi tương tác. Thay vào đó, một kiến trúc CNN hạng nhẹ nhận trực tiếp hộp giới hạn có thể phù hợp hơn với các hệ thống cần chi phí tính toán thấp. Trong kịch bản sử dụng dự kiến, người dùng khoanh vùng sơ bộ, sau đó có thể điều chỉnh hộp giới hạn và nhận lại kết quả suy luận cập nhật.
...
Trong giai đoạn huấn luyện, hộp giới hạn được tạo từ mặt nạ nhãn gốc và biến đổi theo các kịch bản mô phỏng. Trong giai đoạn suy luận, hộp được cung cấp trực tiếp bởi người dùng. Phạm vi thực nghiệm của khóa luận sử dụng câu nhắc mô phỏng có kiểm soát; vì vậy, các kết quả hiện tại phản ánh hành vi của mô hình trong thiết lập đã biết gần đúng vị trí tổn thương, chưa đại diện trực tiếp cho toàn bộ quy trình sử dụng với câu nhắc do bác sĩ vẽ tự do. Việc đánh giá với hộp giới hạn do bác sĩ vẽ trực tiếp được xác định là hướng phát triển tiếp theo.

Bên cạnh bài toán phân đoạn chính, khóa luận thực hiện một thử nghiệm mở rộng với Gatekeeper sử dụng EfficientNet-B3. Mô-đun này cung cấp gợi ý sàng lọc trước khi người dùng xác nhận và cung cấp hộp giới hạn. Gatekeeper không thuộc đóng góp kiến trúc chính và không tự động thay thế quyết định của người dùng.
...
\begin{enumerate}
    ...
    Cổng không gian PSG (\textit{Prompt Spatial Gate}) sử dụng bản đồ nhiệt để điều biến đặc trưng tại các tầng mã hóa. Về bản chất, đây là cơ chế khuếch đại không gian có kết nối dư: đặc trưng tại vùng được chỉ định bởi câu nhắc có thể được tăng cường, trong khi đường truyền của đặc trưng ảnh ban đầu vẫn được giữ lại.
