...
Trong thiết lập phân đoạn tự động, việc bắt mô hình vừa tìm tổn thương vừa phân đoạn nó trên ảnh X-quang xương không hề dễ. Nguyên nhân chính là ảnh thường có độ tương phản thấp, nhiều vùng bị chồng cấu trúc và tổn thương có thể rất nhỏ. Khi chuyển sang phân đoạn tương tác, người dùng có thể cung cấp thêm thông tin không gian, nhưng nếu câu nhắc chỉ được ghép với ảnh ngay từ đầu vào thì về sau tín hiệu này có thể yếu dần qua nhiều tầng tích chập. Vì vậy, một động lực quan trọng của khóa luận là tìm cách đưa câu nhắc vào sâu hơn trong mạng để nó vẫn còn tác dụng ở cả phần mã hóa lẫn giải mã.

Một vấn đề khác cũng rất đáng quan tâm là độ ổn định của mô hình. Trong thực tế, người dùng khó vẽ được một hộp giới hạn lúc nào cũng đẹp và sát hoàn toàn với tổn thương. Hộp có thể rộng hơn cần thiết hoặc hơi lệch tâm. Nếu mô hình bám quá cứng vào đường biên của hộp thì chỉ cần thao tác lệch một chút là kết quả sẽ giảm rõ rệt. Cho nên khóa luận hướng đến một cách biểu diễn câu nhắc mềm hơn và một cơ chế tiếp nhận linh hoạt hơn để mô hình chịu lỗi tốt hơn trong các tình huống hợp lý.

\subsection{Động lực thực tiễn}

Các mô hình nền tảng dựa trên Transformer có nhiều ưu điểm, nhưng thường nặng nề về số tham số và tài nguyên chạy. Nếu muốn triển khai trên phần cứng hạn chế hoặc trong các tình huống cần phản hồi nhanh thì đây là một trở ngại. So với hướng đó, một mô hình CNN gọn hơn nhưng tận dụng tốt hộp giới hạn đầu vào có lẽ hợp hơn cho một hệ thống hỗ trợ. Trong kịch bản mà khóa luận hướng đến, người dùng chỉ cần khoanh sơ bộ vùng nghi ngờ, xem kết quả, rồi nếu cần thì chỉnh lại hộp và chạy lại.
...
\begin{enumerate}
    ...
    Cổng không gian PSG (\textit{Prompt Spatial Gate}) dùng bản đồ nhiệt để điều biến đặc trưng ở các tầng mã hóa. Có thể hiểu đơn giản là mô hình được nhắc phải chú ý nhiều hơn vào vùng mà người dùng đã khoanh, nhưng vẫn giữ đường truyền của đặc trưng ảnh gốc nhờ kết nối dư.

    \item \textbf{Điều kiện hóa bộ giải mã bằng CAD.}

    CAD (\textit{Conditional Attention Decoder}) mở rộng cổng chú ý của Attention U-Net~\cite{oktay2018attentionunet} bằng cách đưa thêm đặc trưng câu nhắc vào tín hiệu điều khiển. Nhờ đó, phần chú ý ở kết nối tắt không chỉ phụ thuộc vào đặc trưng ảnh mà còn phụ thuộc vào vùng người dùng đang quan tâm.

    \item \textbf{Đánh giá thiết kế trên hai bộ dữ liệu và nhiều điều kiện câu nhắc.}

    PGA-UNet được huấn luyện và đánh giá riêng trên BTXRD và FracAtlas, là hai bộ dữ liệu có dạng tổn thương khác nhau gồm khối u xương và gãy xương. Thực nghiệm bao gồm so sánh với U-Net, Attention U-Net và SAM-Med2D; đánh giá trong các kịch bản câu nhắc Bao trọn, Lệch tâm và Hỗn hợp; nghiên cứu loại bỏ thành phần; đánh giá chéo và phân tích theo từng nhóm tổn thương.
...
\begin{itemize}
    ...
    \item mô phỏng sai lệch câu nhắc bằng các phép mở rộng và dịch chuyển hộp giới hạn theo quy tắc có kiểm soát, không thay thế cho đánh giá với câu nhắc thực do người dùng cung cấp;
