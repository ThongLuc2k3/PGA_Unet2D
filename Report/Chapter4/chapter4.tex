...
Để các phần sau dễ theo dõi hơn, mục này trình bày trước về dữ liệu, cách chia tập, cấu hình phần cứng và các siêu tham số huấn luyện. Đây là phần hơi khô nhưng cần thiết, vì nếu thiếu thì phần kết quả phía sau sẽ khó hiểu hơn.

\subsection{Hai bộ dữ liệu ảnh X-quang và tiền xử lý đầu vào}
\label{subsec:dataset}

Khóa luận sử dụng hai bộ dữ liệu X-quang xương công khai và độc lập là BTXRD~\cite{btxrd2024} (\textit{Bone Tumor X-Ray Dataset}) và FracAtlas~\cite{fracatlas2023}. BTXRD tập trung vào khối u xương, còn FracAtlas tập trung vào gãy xương ở nhiều vị trí khác nhau. Việc dùng cả hai bộ dữ liệu giúp mô hình được kiểm tra trên hai dạng tổn thương khác nhau: một bên là vùng tổn thương có hình dạng phức tạp hơn, một bên là các đường gãy thường nhỏ và mảnh hơn.
...
Phần tiền xử lý được áp dụng giống nhau cho cả hai bộ dữ liệu. Ảnh X-quang được đưa về kích thước $512\times512$ hoặc $256\times256$ bằng cách co giãn theo cùng hệ số để giữ nguyên tỉ lệ, sau đó đệm (\textit{padding}) thành khung vuông. Mặt nạ nhị phân được tạo từ tọa độ đa giác trong chú thích và cũng được biến đổi theo đúng quy trình đó. Bản đồ nhiệt câu nhắc được chuẩn hóa theo cùng cách để đồng bộ với ảnh đầu vào. Dữ liệu sau tiền xử lý được dùng thống nhất trong huấn luyện, suy luận và lúc trực quan hóa kết quả.
...
Mục này đánh giá hiệu năng của PGA-UNet theo ba hướng chính: so với các mô hình phân đoạn tự động, so với mô hình nền tảng SAM-Med2D và theo ảnh hưởng của độ phân giải đầu vào. Ngoài độ chính xác phân đoạn, phần này cũng xem mức độ ổn định của mô hình khi câu nhắc bị sai lệch. Trừ khi có nói khác đi, các kết quả ở đây đều phải hiểu trong bối cảnh phân đoạn có hướng dẫn bằng hộp giới hạn, chứ không phải một hệ thống tự động phát hiện và phân đoạn đầy đủ.
...
Bên cạnh các phép so sánh chính, khóa luận còn làm thêm ba nhóm thực nghiệm bổ sung để xem đóng góp của từng thành phần kiến trúc rõ đến đâu. Ba nhóm đó gồm nghiên cứu loại bỏ thành phần, đánh giá chéo 4-fold và phân tích theo đặc tính tổn thương. Các thí nghiệm này được thực hiện độc lập trên cả BTXRD và FracAtlas. Trong mục này chỉ tóm tắt các kết luận quan trọng nhất; số liệu đầy đủ và hình minh họa bổ sung được để ở Phụ lục~\ref{AppendixA}.
...
PGA-UNet cũng được đánh giá bằng 4-fold cross-validation trên cả BTXRD và FracAtlas, trong đó các đa giác thuộc cùng một ảnh luôn được giữ trong cùng một phân vùng. Trên BTXRD, độ lệch chuẩn của Dice qua bốn phân vùng nằm trong khoảng 0,0087 đến 0,0119; còn trên FracAtlas là 0,0064 đến 0,0084. Giá trị Dice trung bình giữa các phân vùng cũng sát với kết quả trên tập kiểm thử độc lập.

Độ lệch chuẩn tương đối thấp trên cả hai bộ dữ liệu cho thấy PGA-UNet tương đối ổn định trước các cách chia dữ liệu đã khảo sát. Dù vậy, kết quả này chưa thay cho việc đánh giá qua nhiều lần khởi tạo ngẫu nhiên khác nhau. Các bảng chi tiết hơn được để ở Phụ lục~\ref{AppendixA}.
...
Chương này đã trình bày toàn bộ phần thực nghiệm của PGA-UNet trên BTXRD và FracAtlas, gồm so sánh với các mô hình cơ sở, so sánh với SAM-Med2D, khảo sát ảnh hưởng của độ phân giải và tổng hợp kết quả từ nghiên cứu loại bỏ thành phần, đánh giá chéo và phân tích theo đặc tính tổn thương. Trong phạm vi phân đoạn có hướng dẫn bằng hộp giới hạn, PGA-UNet cho kết quả tốt hơn các mô hình cơ sở không dùng câu nhắc và cũng cao hơn SAM-Med2D trong phép so sánh cùng điều kiện ở độ phân giải $256\times256$. Mô hình cũng giữ được độ ổn định tốt hơn dưới các sai lệch câu nhắc đã mô phỏng.
