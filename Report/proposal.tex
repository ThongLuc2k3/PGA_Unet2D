...
\begin{document}
    ...
    %Tên đề tài phải VIẾT HOA
        ...
        \textbf{\huge PHÁT TRIỂN HỆ THỐNG PHÂN ĐOẠN ẢNH X-QUANG VỀ XƯƠNG DỰA VÀO CÂU NHẮC TRỰC QUAN} 
        \\
        
    %Tên đề tài bằng tiếng Anh (nếu có)
    \vspace{.5cm}
        \textit{\textbf{\Large (Developing a bone X-ray image segmentation system based on visual prompts)}}
    \end{center}
    ...
    \begin{itemize}
        ...
        \item Xây dựng quy trình \textbf{IPR} (Sequential Prompt Refinement) giúp hệ thống tự động tinh chỉnh câu nhắc, tăng độ sắc nét cho đường biên phân đoạn.
        \item Phát triển cơ chế cứu hộ bằng \textbf{GradCAM} để hỗ trợ tìm kiếm vùng nghi ngờ khi người dùng cung cấp câu nhắc sai lệch hoàn toàn.
    ...
    Tích hợp các mô hình vào giao diện phần mềm. Khi bác sĩ khoanh vùng (Bounding Box), hệ thống chuyển đổi thành câu nhắc trực quan để kích hoạt PGA-UNet. Kết quả sau đó đi qua module kiểm duyệt \textbf{IPR}: nếu câu nhắc bị lệch, thuật toán sẽ tự động dịch chuyển tâm để nắn sát lại mục tiêu. Nếu câu nhắc sai hoàn toàn, hệ thống sử dụng \textbf{GradCAM} để gợi ý lại vùng tổn thương cho bác sĩ.

    \vspace{.5cm}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{images/Overview.pdf}
        \caption{Sơ đồ kiến trúc tổng thể của hệ thống đề xuất (PGA-UNet và IPR Pipeline)}
        \label{fig:sodo_pga}
    \end{figure}
    \vspace{.5cm}
    ...
    \begin{itemize}
        \item \textbf{Định lượng:} Mô hình PGA-UNet đạt chỉ số Dice Similarity Coefficient (DSC) $\approx$ 86\% và IoU $\approx$ 76\%, vượt xa các Baseline U-Net ($\approx$ 38\%) trên cùng tập dữ liệu kiểm thử 248 ảnh.
        \item \textbf{Định tính:} Khả năng chịu lỗi (fault tolerance) tốt trước các sai số của người dùng nhờ thuật toán IPR. Tốc độ suy luận đạt mức thời gian thực (< 1 giây/ảnh).
    ...
    23/03 - 30/04/2026 & Phát triển module IPR và GradCAM để tăng tính bền bỉ cho hệ thống. & Nguyễn Hữu Bình, Thông Lúc \\ \hline
    
    01/05 - 20/05/2026 & Tinh chỉnh siêu tham số, huấn luyện với hàm mất mát kết hợp BCE-Dice. & Nguyễn Hữu Bình, Thông Lúc \\ \hline
    
    21/05 - 05/06/2026 & Đánh giá hiệu năng trên 248 ảnh test, so sánh với Baseline U-Net. & Nguyễn Hữu Bình, Thông Lúc \\ \hline
