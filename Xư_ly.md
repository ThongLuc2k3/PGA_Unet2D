...
Phạm vi đã được rút gọn theo quyết định của người dùng: bỏ C.3, C.5, C.6, C.7, C.8, C.9 (xem lý do ở mục "Đã bỏ khỏi phạm vi" bên dưới); C.4 đổi hướng thành sửa report thay vì thêm ablation. **Còn lại 7 mục active, chưa làm mục nào (0/7): C.1, C.2, C.4 (dạng mới), C.10, C.11, C.12, C.13.** Khác với A/B/D/E (chủ yếu sửa văn bản), phần lớn nhóm C còn lại là **thực nghiệm thật** — cần chạy notebook, GPU (Kaggle/Colab). Xếp theo độ sẵn sàng (dễ → khó) để tiện ước lượng công sức trước khi bắt đầu.
...
**Đã bổ sung xong export CSV per-image** (`results/..._per_image.csv`) vào **5 notebook test** (không cần train lại, chỉ cần chạy lại suy luận để lấy file mới — trừ notebook V4 là phải train từ đầu vì biến thể mới):
- `Result/Result_BTXRD/Ablation/test-v1-nopsg-nocad-concat.ipynb` (đã có checkpoint sẵn)
- `Result/Result_BTXRD/Ablation/test-v2-psg-only.ipynb` (đã có checkpoint sẵn)
- `Result/Result_BTXRD/Ablation/test-v3-cad-only.ipynb` (đã có checkpoint sẵn)
- `Result/Result_BTXRD/Ablation/test-v4-nopsg-nocad-binary.ipynb` (mới tạo — **cần train trước**, xem D.4)
- `Result/Result_BTXRD/pga-vs-unet2d-r512.ipynb` (đã có checkpoint sẵn, chỉ export phần PGA-UNet)

Lý do thêm cả V2/V3/PGA@512 dù hiện tại chỉ kiểm định V1 vs V4: dữ liệu per-image có sẵn phòng khi cần kiểm định thêm cặp khác sau này (PSG-alone, CAD-alone, PGA vs CAD-only...) mà không phải sửa notebook lại từ đầu — chi phí thêm gần như bằng 0 vì đã có checkpoint, chỉ tốn thời gian chạy lại suy luận (không train).

**Việc còn lại của người dùng:** (1) train `V4_NoPSG_NoCAD_Binary.ipynb` trên Colab/Kaggle → upload checkpoint lên Drive → điền `CKPT_ID` vào `test-v4-nopsg-nocad-binary.ipynb`; (2) chạy lại cả 5 notebook test ở trên (4 cái còn lại chỉ cần chạy lại suy luận, không train) để có đủ file `*_per_image.csv`; (3) đưa file CSV cho tôi — tôi viết đoạn `scipy.stats.wilcoxon` và cập nhật lại Bảng ablation + Hình 4.x trong Chapter 4.
...
*Chuẩn bị:* không cần thực nghiệm, chỉ cần đọc thêm paper — cần xác nhận paper nào thực sự nên trích trước khi đưa vào bài.

**C.13 — Tìm kiếm bài báo liên quan có kiến trúc tương tự PSG/CAD/PGA-UNet** *(nhiệm vụ mới bổ sung)*
Chủ động tìm càng nhiều càng tốt các bài báo/công trình đã công bố có kiến trúc/ý tưởng tương tự: (a) cơ chế "gate" không gian điều biến đặc trưng encoder theo tín hiệu câu nhắc/mask (tương tự PSG), (b) cơ chế attention ở decoder được điều kiện hóa bởi đặc trưng câu nhắc (tương tự CAD), (c) kiến trúc kết hợp cả hai dạng tại encoder lẫn decoder cho bài toán segmentation dựa trên prompt dạng hộp giới hạn (tương tự tổng thể PGA-UNet). Mục tiêu: xác định rõ ranh giới cái gì đã có người làm, cái gì thực sự mới, làm nền tảng trực tiếp và cụ thể hơn cho C.2/A.5 (hạ/giữ mức claim novelty đúng mực). Việc này thuộc dạng đọc/tổng hợp tài liệu, không cần thực nghiệm hay GPU.
...
*Cập nhật (quyết định mới của người dùng):* **bỏ hẳn biến thể V4 cũ "PGA-UNet (Binary)"** (PSG✓ CAD✓ Prompt=Binary) khỏi bảng ablation. **Thay bằng biến thể mới "U-Net+Binary"** (PSG✗ CAD✗ Prompt=Binary — cùng kiến trúc concat đơn giản như U-Net+Concat/V1, chỉ khác kênh input là mask nhị phân thay vì Gaussian heatmap). Hệ quả: bảng ablation không còn phép so sánh Binary-vs-Gaussian ở kiến trúc đầy đủ PSG+CAD nữa, chỉ còn ở mức baseline (U-Net+Concat vs U-Net+Binary).

Bảng ablation mới (5 hàng): U-Net+Concat (Gaussian) / **U-Net+Binary (MỚI)** / U-Net+PSG / U-Net+CAD / PGA-UNet (đề xuất).

**Trạng thái: ⏳ CHỜ THỰC NGHIỆM MỚI** — "U-Net+Binary" chưa từng được train, cần chạy thật trước khi sửa lại Bảng~`tab:ablation_arch`, Hình~`fig:chart_ablation`, và văn bản phân tích liên quan trong chapter4.tex. Đã chuẩn bị sẵn 2 notebook để người dùng chạy trên Colab/Kaggle:
- `Source/File_Train/Ablation/V4_NoPSG_NoCAD_Binary.ipynb` — train từ đầu (copy từ `V1_NoPSG_NoCAD_Concat.ipynb`, chỉ đổi `BINARY_PROMPT=True`), 100 epoch, patience 15, cùng hyperparameter với V1.
- `Result/Result_BTXRD/Ablation/test-v4-nopsg-nocad-binary.ipynb` — test/đánh giá image-level (copy từ `test-v1-nopsg-nocad-concat.ipynb`, đổi `BINARY_PROMPT=True`, `CKPT_ID` để trống chờ điền checkpoint mới). Đã bổ sung thêm export CSV **per-image** (`results/..._per_image.csv`) trong notebook test này — dữ liệu này dùng luôn cho C.1 (Wilcoxon U-Net+Concat vs U-Net+Binary) sau khi có kết quả, không cần sửa thêm.

Các file V4 cũ (`V4_Full_BinaryPrompt.ipynb`, `test-v4-full-binaryprompt.ipynb`) **giữ nguyên, không xóa** (lưu vết lịch sử thực nghiệm), nhưng không còn dùng số liệu từ 2 file này trong báo cáo nữa.
...
1. **D.4 (việc mới phát sinh)** — train biến thể "U-Net+Binary" (đã có sẵn `V4_NoPSG_NoCAD_Binary.ipynb` để train + `test-v4-nopsg-nocad-binary.ipynb` để test), rồi cập nhật lại bảng/hình ablation trong Chapter 4.
2. **C.1** — sau khi có U-Net+Binary ở trên, chạy lại 4 notebook test còn lại (V1, V2, V3, PGA@512 — chỉ suy luận, không train) để lấy đủ CSV per-image, đưa cho tôi tính Wilcoxon (U-Net+Concat vs U-Net+Binary).
...
4. **C.10** — chạy ablation (5 biến thể, gồm cả U-Net+Binary mới) trên FracAtlas.
