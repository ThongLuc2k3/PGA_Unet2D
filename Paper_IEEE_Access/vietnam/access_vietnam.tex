...
MedSAM và ScribblePrompt-UNet chạy ở $1024$ và $128$ gốc nên chúng tôi so các độ đo bất biến theo khung. PGA-UNet ở cấu hình tốt nhất $512$ (0.782 trên BTXRD) dẫn trước cả MedSAM fine-tune (0.724) lẫn ScribblePrompt-UNet (0.653). Trên FracAtlas với prompt bao phủ, MedSAM fine-tune đạt 0.729, ngang PGA-UNet-512 (0.727), nhưng phải chạy ở đầu vào $1024$, gấp khoảng bốn lần diện tích pixel và 32 lần số tham số; khi prompt lệch tâm, PGA-UNet (0.713) vượt MedSAM (0.666) và dẫn về CBL ở mọi ô. ScribblePrompt-UNet, với số tham số xấp xỉ PGA-UNet (4,0 triệu so với 3,0 triệu), vẫn kém hơn từ 0.05 đến 0.13 Dice và có khoảng cách bao-phủ-so-với-lệch-tâm lớn nhất trong nhóm. Sự thay đổi từ prompt bao phủ sang lệch tâm được bàn riêng ở Mục V.F.

\begin{table}[H]
\caption{So sánh với các baseline nhận prompt, merge ở mức ảnh, prompt bao phủ. PGA-UNet trình bày ở $256$ (khớp độ phân giải với SAM-Med2D) và ở $512$ (cấu hình tốt nhất); MedSAM và ScribblePrompt-UNet chạy ở $1024$ và $128$ gốc. HD95 thô chấm trong khung riêng của từng mô hình nên không so được giữa các dòng và không in đậm; Dice, IoU và CBL bất biến theo khung.}
\label{tab:sam}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
