...
Vậy khu trú là một phần lớn của độ khó ở đây, nhưng có hộp chưa phải toàn bộ câu chuyện. Cách PGA-UNet tiêu thụ hộp đó mua thêm một biên nhất quán so với một mạng quy ước nhận cùng đầu vào.
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{../images/results/unet_extreme_groups_btxrd.png}
\caption{So sánh định tính giữa Attention U-Net và PGA-UNet trên BTXRD cho một ca nhóm Top-Dice (IMG000184) và một ca nhóm Bottom-Dice (IMG000791) của Attention U-Net. Các hàng dùng cùng stem ảnh; với PGA-UNet, panel ảnh đầu vào và panel hộp prompt được gộp làm một.}
\label{fig:extreme}
\end{figure}
...
Nếu việc giữ bằng chứng yếu sống qua mạng có ý nghĩa ở đâu, thì là ở đây. Tập con tổn thương nhỏ là 50 ảnh test mỗi bộ dữ liệu có tổng diện tích tổn thương nhỏ nhất, 7 đến 433 pixel trên BTXRD và 145 đến 944 trên FracAtlas. Mọi số được chấm trong một khung $512\times512$ chung.
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{../images/results/small_lesion_sam_pga256_pga512.png}
\caption{So sánh tổn thương nhỏ tiêu biểu trên BTXRD (IMG000868, hai đa giác nhỏ): SAM-Med2D đã fine-tune ở $256\times256$, PGA-UNet ở $256\times256$, và PGA-UNet ở $512\times512$. Panel ảnh đầu vào và panel hộp prompt được gộp làm một.}
\label{fig:small}
\end{figure}

Đọc hẹp, các tổn thương nhỏ nhất là ca thử thách cho toàn bộ cách tiếp cận, và cả điều kiện hóa prompt lẫn độ phân giải đầu vào đều đáng giá hơn ở đây so với trên toàn tập.

\subsection{Độ nhạy với dịch prompt}
\label{sec:robust}
Bảng~\ref{tab:robust} dịch hộp lệch tâm theo một quy tắc cố định. PGA-UNet mất 0.008 Dice trên BTXRD và 0.014 trên FracAtlas, với CBL giảm 0.006 và 0.015; trên tập con tổn thương nhỏ, mức mất giữ dưới 0.01 trên cả hai. Các baseline prompt quy ước mất một lượng tương đương hoặc hơi lớn hơn, và SAM-Med2D đã fine-tune khoảng 0.03. Đây là độ nhạy với một quy tắc dịch đã cố định trước. Nó không nói gì về hộp tùy ý do bác sĩ vẽ, và hộp phủ một phần hay sai vùng chưa bao giờ được thử.
...
\caption{Hành vi tiêu biểu khi nhiễu loạn prompt trên cả hai bộ dữ liệu. Các vết gãy mảnh vẫn khó hơn tổn thương gọn ngay cả khi hộp vẫn phủ mục tiêu.}
\label{fig:robust}
\end{figure}
...
\caption{So sánh tiêu biểu giữa biểu diễn prompt Gaussian dạng cao nguyên và một hộp nhị phân trong kiến trúc đầy đủ.}
...
\caption{Ca lỗi tiêu biểu ở vùng giải phẫu chồng lấn. Ngay cả với một hộp bao phủ, biên vẫn khó vì các cấu trúc cục bộ bị nhầm lẫn về mặt thị giác.}
\label{fig:failure}
\end{figure}
