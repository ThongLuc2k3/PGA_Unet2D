...
Monte Carlo cross-validation ở trên thay đổi tập chia mức ảnh trong khi giữ nguyên seed huấn luyện ở 22120196. Để đặc trưng hóa nguồn biến thiên bổ sung, tức chính sự ngẫu nhiên của lượt huấn luyện, chúng tôi đang huấn luyện lại riêng PGA-UNet ở $512\times512$ trên đúng tập chia cố định gốc (Bảng~\ref{tab:splits}), dùng thêm ba seed huấn luyện khác, mọi cài đặt còn lại từ Mục~\ref{sec:training} giữ nguyên. Cả bốn lượt chạy dùng chung một tập chia train/validation/test cố định; chỉ seed chi phối khởi tạo trọng số, xáo trộn dữ liệu, và các hành vi ngẫu nhiên khác trong huấn luyện là thay đổi giữa chúng.

Phân tích này chỉ giới hạn ở PGA-UNet. Không baseline nào đã đánh giá, hai baseline prompt quy ước hay ba mô hình nền tảng nhận prompt, được huấn luyện lại qua nhiều seed, nên không có so sánh nào trong bài này khẳng định ưu thế ổn định qua các seed huấn luyện so với các baseline đó. Trong ba seed bổ sung, một seed (19122003) đã chạy xong trên cả hai bộ dữ liệu tại thời điểm viết bản nháp này; hai seed còn lại vẫn đang huấn luyện. Vì vậy Bảng~\ref{tab:seedstability} báo cáo kết quả tạm thời với $n=2$ seed đã hoàn tất (22120196 và 19122003), chưa phải đủ bốn; cỡ mẫu được ghi rõ trong caption, và bảng sẽ được bổ sung thêm hai seed còn lại, không tính lại từ đầu, khi chúng hoàn tất (xem \texttt{PENDING\_MULTISEED.md}).

\begin{table*}[t]
\caption{Ổn định theo seed huấn luyện của PGA-UNet ở $512\times512$ trên tập chia cố định gốc (Bảng~\ref{tab:splits}). Kết quả tạm thời từ $n=2$ seed huấn luyện đã hoàn tất (22120196 và 19122003) trong tổng số bốn seed dự kiến; trung bình $\pm$ độ lệch chuẩn mẫu ($\mathrm{ddof}=1$). Hai seed bổ sung còn lại vẫn đang huấn luyện và chưa có trong bảng này. Precision và Recall là giá trị chuẩn ở mức pixel cho mask đã merge mức ảnh, chỉ báo cáo trong bảng này; đây không phải Recall cấp tổn thương (lesion-level), thứ bài này không đo. Xem \texttt{PENDING\_MULTISEED.md}.}
...
\midrule
\multirow{2}{*}{BTXRD}
& Bao phủ (\texttt{center\_zoom}) & $0.771\pm0.016$ & $0.645\pm0.022$ & $0.745\pm0.024$ & $0.831\pm0.000$ & $23.6\pm1.2$ & $0.914\pm0.005$ \\
& Lệch tâm (\texttt{center\_shift}) & $0.762\pm0.017$ & $0.637\pm0.021$ & $0.747\pm0.024$ & $0.819\pm0.002$ & $24.8\pm0.5$ & $0.907\pm0.007$ \\
\midrule
\multirow{2}{*}{FracAtlas}
& Bao phủ (\texttt{center\_zoom}) & $0.728\pm0.001$ & $0.586\pm0.002$ & $0.678\pm0.008$ & $0.835\pm0.011$ & $10.6\pm0.1$ & $0.908\pm0.006$ \\
& Lệch tâm (\texttt{center\_shift}) & $0.713\pm0.001$ & $0.570\pm0.000$ & $0.674\pm0.007$ & $0.806\pm0.004$ & $11.0\pm0.8$ & $0.899\pm0.002$ \\
\bottomrule
\end{tabular}
\end{table*}

Với chỉ hai seed đã hoàn tất, các độ lệch chuẩn này chưa đủ để phản ánh mức biến thiên seed-với-seed thật sự và không nên đọc như một ước lượng ổn định; chúng chỉ mô tả đúng hai lượt chạy đã quan sát được. Đọc như một so sánh hai điểm, không phải một phân bố: Dice của seed mới hoàn tất thấp hơn khoảng 0,02 so với seed gốc trên BTXRD ở cả hai điều kiện prompt, trong khi trên FracAtlas hai seed chênh nhau trong vòng 0,002 Dice. Giữa hai seed đã hoàn tất, Precision chênh khoảng 0,034 trên BTXRD ở cả hai điều kiện prompt và chênh 0,010 đến 0,012 trên FracAtlas, trong khi Recall chênh 0,001 đến 0,002 trên BTXRD và 0,006 đến 0,016 trên FracAtlas. HD95 chênh khoảng 1 đến 2 pixel ở BTXRD bao phủ và khoảng 1 pixel ở FracAtlas lệch tâm, dưới 1 pixel ở hai dòng còn lại. Bức tranh không đồng nhất giữa các độ đo: Dice và Precision chênh nhiều hơn giữa hai seed trên BTXRD so với FracAtlas, trong khi Recall lại chênh ít hơn trên BTXRD so với FracAtlas. Liệu khuôn mẫu nào trong số này có giữ vững hay không thì phải chờ hai seed còn lại; hiện tại chưa có gì ủng hộ một tuyên bố tổng quát về việc bộ dữ liệu hay độ đo nào nhạy hơn với seed huấn luyện.
