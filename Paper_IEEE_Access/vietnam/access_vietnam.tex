...
Ba mô hình nền tảng nhận prompt được thử với cùng một hộp: SAM-Med2D (đưa vào y khoa qua fine-tune diện rộng), MedSAM (fine-tune y khoa, chỉ nhận hộp), và ScribblePrompt-UNet (một UNet nhẹ, không phải kiến trúc SAM). Ở chế độ zero-shot, cả ba đều gần như không dùng được trên X-quang xương: Dice chỉ 0.25 đến 0.43, với precision rất thấp (0.16 đến 0.34) do mask phình ra ngoài tổn thương. Fine-tune trên cùng tập chia và cùng protocol prompt kéo Dice lên thêm 0.24 đến 0.42 (Bảng~\ref{tab:sam}). Sau fine-tune, trên BTXRD PGA-UNet ở $512$ (0.782) dẫn trước cả ba (MedSAM 0.724, ScribblePrompt 0.653, SAM-Med2D 0.630). Trên FracAtlas với prompt bao phủ, MedSAM fine-tune (0.729) ngang PGA-UNet-512 (0.727); nhưng khi prompt lệch tâm, PGA-UNet (0.713) bỏ xa MedSAM (0.666), và PGA-UNet dẫn về CBL ở mọi ô cùng về HD95 ở hầu hết ô, trong khi chỉ dùng khoảng $1/90$ số tham số của MedSAM và một ảnh đầu vào nhỏ hơn. ScribblePrompt-UNet, tuy có số tham số xấp xỉ PGA-UNet, vẫn kém hơn từ 0.05 đến 0.13 Dice và có khoảng cách bao-phủ-so-với-lệch-tâm lớn nhất trong nhóm. Mỗi mô hình nền tảng được đánh giá ở độ phân giải cố định riêng của nó, nên so sánh này không đồng nhất tuyệt đối về độ phân giải. Sự thay đổi từ prompt bao phủ sang lệch tâm được bàn riêng ở Mục V.F.

\begin{table}[H]
\caption{So sánh với các baseline nhận prompt, merge ở mức ảnh, prompt bao phủ. Mỗi mô hình nền tảng chạy ở độ phân giải cố định của nó; PGA-UNet ở $512$.}
\label{tab:sam}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
\midrule
\multirow{7}{*}{BTXRD}
& PGA-UNet             & 512  & \best{0.782} & \best{0.661} & \best{22.8} & \best{0.918} \\
...
& SAM-Med2D fine-tuned & 256 & 0.563 & 0.417 & \best{8.0} & 0.846 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{table}[H]
\caption{Độ nhạy với dịch prompt. Mọi mô hình dưới prompt bao phủ (z) và lệch tâm (s). Attention U-Net (không prompt) bất biến với prompt. Các dòng AttUNet và PGA-UNet ở $512\times512$; các mô hình nền tảng nhận prompt ở độ phân giải gốc của chúng (SAM-Med2D $256$, MedSAM $1024$, ScribblePrompt $128$), merge ở mức ảnh.}
\label{tab:robust}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
