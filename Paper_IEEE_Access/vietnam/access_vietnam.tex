...
Chúng tôi dùng Attention U-Net không prompt làm mô hình tham chiếu tự động, và SAM-Med2D, MedSAM cùng ScribblePrompt làm các mô hình tham chiếu có prompt. Thực nghiệm trên hai bộ dữ liệu BTXRD và FracAtlas, ở ba độ phân giải 128, 256 và 512, dưới hai kiểu hộp: bao phủ và lệch tâm có kiểm soát. Kết quả cho thấy phân đoạn không prompt là bài toán rất khó: Attention U-Net chỉ đạt 0.52 và 0.34 Dice trên hai bộ dữ liệu. Khi được cấp cùng một hộp, PGA-UNet đạt ngang hoặc vượt cả ba mô hình nền tảng nhận prompt đã fine-tune lẫn hai baseline quy ước có prompt, với ưu thế rõ dưới prompt lệch tâm. Khoảng cách này rộng nhất khi so với SAM-Med2D và trên nhóm tổn thương nhỏ nhất: ở đó SAM-Med2D fine-tune tụt xuống còn 0.26 đến 0.37 Dice, trong khi PGA-UNet vẫn giữ được 0.65 đến 0.72. Kết quả ổn định qua nhiều lần chia dữ liệu ngẫu nhiên, và PGA-UNet nhẹ hơn SAM-Med2D khoảng 92 lần về số tham số. Hệ thống cũng kèm hai tín hiệu hỗ trợ không cần nhãn, tính từ đầu ra của mô hình: một điểm tự đánh giá độ tin cậy của mask và một danh sách ngắn vùng ứng viên, cả hai ở mức thăm dò.
...
Ngoài các đóng góp trên, hệ thống còn lộ ra hai tín hiệu hỗ trợ không cần nhãn, tính trực tiếp từ đầu ra của mô hình đã huấn luyện: một điểm tự đánh giá xếp hạng độ tin cậy của mask, và một danh sách ngắn vùng ứng viên. Cả hai được trình bày ở mức thăm dò, không phải là đóng góp chính của bài.
...
Bài báo này bắt đầu từ chính khoảng trống đó. Trên ảnh X-quang xương thang xám, biết được tổn thương nằm khoảng đâu mới chỉ giải quyết được một nửa vấn đề; phần khó hơn nằm ở việc giữ lại bằng chứng mờ nhạt trong khi vẫn lọc bỏ nhiễu, sau khi không gian tìm kiếm đã được thu hẹp. Chúng tôi cho rằng một mạng CNN nhẹ có định hướng bằng prompt có thể làm tốt việc này hơn, nếu giữ cho prompt luôn hoạt động liên tục từ tầng encoder đầu tiên cho đến tầng decoder cuối cùng. Trong bài báo, chúng tôi khảo sát trực tiếp bối cảnh đó, dùng SAM-Med2D, MedSAM, và ScribblePrompt làm các mô hình tham chiếu nhận prompt.
...
Bên cạnh mask dự đoán, PGA-UNet còn trả về một số vô hướng $Q$, ước lượng mức độ đáng tin của dự đoán mà không cần đến đáp án thật. $Q$ không phải một đại lượng được học; nó chỉ được tính từ chính đầu ra của mô hình lúc suy luận, là trung bình cộng của hai chỉ dấu. $Q$ được dùng như một tín hiệu hỗ trợ, giúp người dùng soi kỹ hơn một mask cụ thể, chứ không phải một thành phần cốt lõi của phương pháp.
...
Ba mô hình nền tảng nhận prompt được thử với cùng một hộp: SAM-Med2D (đưa vào y khoa qua fine-tune diện rộng), MedSAM (fine-tune y khoa, chỉ nhận hộp), và ScribblePrompt-UNet (một UNet nhẹ, không phải kiến trúc SAM). Ở chế độ zero-shot, cả ba đều gần như không dùng được trên X-quang xương: Dice chỉ 0.25 đến 0.43, với precision rất thấp (0.16 đến 0.34) do mask phình ra ngoài tổn thương. Fine-tune trên cùng tập chia và cùng protocol prompt kéo Dice lên thêm 0.24 đến 0.42 (Bảng~\ref{tab:sam}).

Ở thiết lập khớp độ phân giải, PGA-UNet ở $256$ dẫn trước SAM-Med2D fine-tune ở $256$ đúng 0.132 Dice trên BTXRD (0.762 so với 0.630) và 0.066 trên FracAtlas (0.629 so với 0.563), với CBL cũng cao hơn (0.902 so với 0.836; 0.895 so với 0.846).

MedSAM và ScribblePrompt-UNet chạy ở $1024$ và $128$ gốc nên chúng tôi so các độ đo bất biến theo khung. PGA-UNet ở cấu hình tốt nhất $512$ (0.782 trên BTXRD) dẫn trước cả MedSAM fine-tune (0.724) lẫn ScribblePrompt-UNet (0.653). Trên FracAtlas với prompt bao phủ, MedSAM fine-tune đạt 0.729, ngang PGA-UNet-512 (0.727), nhưng phải chạy ở đầu vào $1024$, gấp khoảng bốn lần diện tích pixel và 32 lần số tham số; khi prompt lệch tâm, PGA-UNet (0.713) vượt MedSAM (0.666) và dẫn về CBL ở mọi ô. ScribblePrompt-UNet, với số tham số xấp xỉ PGA-UNet (4,0 triệu so với 3,0 triệu), vẫn kém PGA-UNet-512 từ 0.05 đến 0.13 Dice và có khoảng cách bao-phủ-so-với-lệch-tâm lớn nhất trong nhóm. Tuy nhiên, ở độ phân giải khớp $128$, PGA-UNet (0.714 trên BTXRD, 0.596 trên FracAtlas) không thắng đều: PGA-UNet dẫn rõ trên BTXRD và về HD95 ở cả hai bộ dữ liệu (5.8 so với 122.7; 8.1 so với 17.9), nhưng ScribblePrompt-UNet lại dẫn trên FracAtlas ở Dice, IoU và CBL (0.679 so với 0.596; 0.527 so với 0.445; 0.891 so với 0.849). Khoảng cách của PGA-UNet với ScribblePrompt-UNet ở $512$ vì vậy phản ánh độ phân giải vận hành cao hơn không kém gì kiến trúc, giống hệt cách MedSAM hòa được PGA-UNet ở trên nhờ lợi thế độ phân giải và tham số theo chiều ngược lại. Sự thay đổi từ prompt bao phủ sang lệch tâm được bàn riêng ở Mục V.F.

\begin{table}[H]
\caption{So sánh với các baseline nhận prompt, merge ở mức ảnh, prompt bao phủ. PGA-UNet trình bày ở $512$ (cấu hình tốt nhất), $256$ (khớp độ phân giải với SAM-Med2D), và $128$ (khớp độ phân giải với ScribblePrompt-UNet); MedSAM chạy ở $1024$ gốc. HD95 thô chấm trong khung riêng của từng mô hình nên không so được giữa các dòng và không in đậm; Dice, IoU và CBL bất biến theo khung.}
\label{tab:sam}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
\midrule
\multirow{9}{*}{BTXRD}
& PGA-UNet             & 512  & \best{0.782} & \best{0.661} & 22.8 & \best{0.918} \\
...
\midrule
\multirow{9}{*}{FracAtlas}
& PGA-UNet             & 512  & 0.727 & \best{0.585} & 10.5 & \best{0.913} \\
& PGA-UNet             & 256  & 0.629 & 0.470 & 10.6 & 0.895 \\
& PGA-UNet             & 128  & 0.596 & 0.445 & 8.1 & 0.849 \\
& MedSAM zero-shot     & 1024 & 0.319 & 0.193 & 60.1 & 0.843 \\
& MedSAM fine-tuned    & 1024 & \best{0.729} & 0.583 & 16.2 & 0.900 \\
...
\caption{Ca tổn thương nhỏ trên BTXRD (IMG000868, 2 đa giác GT), Attention U-Net (không prompt) so với PGA-UNet, cùng ở $512\times512$. Không có prompt, baseline tự động gần như bỏ sót cả hai tổn thương (Dice 0.085); với cùng hộp, PGA-UNet khôi phục được chúng (Dice 0.720). Hai baseline Attention U-Net khớp prompt còn lại đã có số liệu ở Bảng~\ref{tab:small-att}, không lặp lại ở hình để dễ đọc.}
\label{fig:small-att}
\end{figure}
...
\label{sec:robust}
Bảng~\ref{tab:robust} trình bày kết quả khi hộp bị dịch lệch tâm theo một quy tắc cố định. PGA-UNet chỉ mất 0.008 Dice trên BTXRD và 0.014 trên FracAtlas, với CBL giảm tương ứng 0.006 và 0.015; riêng trên tập con tổn thương nhỏ, mức mất vẫn giữ dưới 0.01 ở cả hai bộ dữ liệu. Các baseline prompt quy ước mất một lượng tương đương hoặc nhỉnh hơn một chút, còn ba mô hình nền tảng nhận prompt sau fine-tune mất từ 0.03 đến 0.08 Dice, ScribblePrompt-UNet nhiều nhất. Đây chỉ là độ nhạy đối với một quy tắc dịch đã cố định từ trước. Chúng tôi cũng chưa thử hộp phủ một phần hay hộp sai vùng, nên con số này không nói được gì về hộp do bác sĩ tự vẽ tùy ý ngoài đời.

\begin{table}[H]
\caption{Độ nhạy với dịch prompt. Mọi mô hình dưới prompt bao phủ (z) và lệch tâm (s). Attention U-Net (không prompt) bất biến với prompt. Các dòng AttUNet và PGA-UNet ở $512\times512$; các mô hình nền tảng nhận prompt ở độ phân giải gốc của chúng (SAM-Med2D $256$, MedSAM $1024$, ScribblePrompt $128$), merge ở mức ảnh. HD95 thô trải trên các khung gốc khác nhau nên không so được giữa các họ mô hình và không in đậm; Dice, $\Delta$Dice, IoU và CBL bất biến theo khung.}
\label{tab:robust}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
& PGA-UNet & \best{0.727 / 0.713} & $-0.014$ & \best{0.585 / 0.570} & 10.5 / 10.4 & \best{0.913 / 0.897} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\label{sec:ablation}
Thí nghiệm ablation lần lượt bỏ đi từng yếu tố khỏi mô hình đầy đủ (prompt Gaussian kết hợp PSG và CAD), đồng thời thử riêng việc thay prompt Gaussian bằng một hộp nhị phân (Bảng~\ref{tab:ablation}). Chúng tôi không đọc nó như một cuộc tìm cấu hình đạt đỉnh, mà là tìm cấu hình bám sát nhóm dẫn đầu trên cả hai bộ dữ liệu. Mô hình đầy đủ đứng nhất hoặc nhì ở cả bốn tổ hợp dataset và điều kiện prompt; mỗi biến thể chỉ lọt top-2 đúng một tổ hợp. Mọi cấu hình ngang bằng hoặc vượt mô hình đầy đủ trên BTXRD đều trả lại phần lớn phần hơn đó trên FracAtlas: biến thể hộp nhị phân dẫn BTXRD bao phủ 0.006 Dice nhưng kém 0.059 trên FracAtlas, còn "chỉ PSG" sát BTXRD nhưng tụt 0.092 Dice dưới mô hình đầy đủ trên FracAtlas. Mô hình đầy đủ cũng có khoảng cách Dice BTXRD so với FracAtlas nhỏ nhất trong mọi cấu hình (0.055 so với 0.075 đến 0.144 của các cấu hình khác). Đây mới là kết quả trên một tập chia.
...
\begin{table}[H]
\caption{So sánh bắt cặp mô hình đầy đủ với từng biến thể, Dice từng ảnh trên một tập chia. $\Delta$ là chênh Dice trung bình từng ảnh (đầy đủ trừ biến thể), kiểm định hoán vị bắt cặp đổi dấu hai phía ($20{.}000$ lần). $^{**}$ đánh dấu ô còn ý nghĩa ($p<0{,}05$) sau hiệu chỉnh Holm trên toàn bộ họ kiểm định ablation (bốn độ đo $\times$ hai bộ dữ liệu $\times$ hai điều kiện prompt); $^{*}$ đánh dấu $p<0{,}05$ thô không sống sót hiệu chỉnh.}
...
Các mức chênh ablation một-tập-chia này khớp với phần còn lại của nghiên cứu. Prompt Gaussian và nhánh CAD là hai thứ bỏ đi tốn nhất trên FracAtlas, bộ dữ liệu khó hơn và nhỏ hơn, nơi cả hai mức sụt đều sống sót hiệu chỉnh Holm trên toàn bộ họ kiểm định ablation (Bảng~\ref{tab:ablation-sig}); biến thể bỏ CAD mất 0.09 đến 0.10 Dice. Trên BTXRD, nơi mọi cấu hình vốn đã quanh 0.78, không chênh lệch nào đạt ý nghĩa ở bất kỳ độ đo nào, kể cả mức 0.006 Dice mà biến thể hộp nhị phân dẫn trước (khoảng tin cậy bootstrap $95\%$ $[-0{,}017; +0{,}004]$). Đây cũng chính là chiều hướng thấy ở kết quả dịch prompt (Mục~\ref{sec:robust}) và ở nhóm Bottom-Dice của Attention U-Net (Bảng~\ref{tab:extreme}): việc điều kiện hóa giúp ích ở nơi tín hiệu định vị yếu và ít tác dụng ở nơi tín hiệu đã mạnh. Vì vậy chúng tôi xem ablation như một mạch bằng chứng hội tụ, không phải một chứng minh từng thành phần, điều mà một tập chia dù sao cũng không cho được.
...
Điểm $Q$ mặc định rằng trong hộp có tổn thương. Để soi lại giả định này, chúng tôi chạy đúng quy trình 50 hộp ở trên nhưng trên các ảnh lành của mỗi bộ dữ liệu (1879 ảnh không tổn thương của BTXRD, 3366 ảnh không gãy của FracAtlas), nơi không có độ phủ để tính (Bảng~\ref{tab:boxquality-normal}). $Q$ chưa ở mức thấp một cách đáng tin: trung vị sụt về gần 0, nhưng vẫn còn khoảng 28\% hộp trên BTXRD và 42\% hộp trên FracAtlas vượt ngưỡng $Q = 0.5$, và hầu như ảnh lành nào cũng có một cách đặt hộp cho điểm cao (trung bình của giá trị $Q$ lớn nhất mỗi ảnh là 0.85 và 0.90). Kết quả này một phần đến từ chỗ trọng số của mô hình chỉ được học trên ảnh có tổn thương, không có lớp âm tính, nên với một hộp bất kỳ nó vẫn phân đoạn dứt khoát và ổn định phần cấu trúc nằm trong hộp, còn $Q$ thì thưởng đúng cho hành vi đó. Vì vậy $Q$ chỉ nên hiểu là một tín hiệu chất lượng có điều kiện: nó xếp hạng độ tốt của một mask sau khi tổn thương đã được khoanh vùng, chứ tự nó chưa báo được việc không có tổn thương. Đưa thêm ảnh lành vào huấn luyện là hướng chúng tôi sẽ phát triển tiếp.
...
Câu hỏi bài báo đặt ra chưa bao giờ đơn thuần là prompt có ích hay không nói chung. Câu hỏi thật sự là: một khi hộp thô đã thu hẹp việc tìm kiếm, cách mô hình xử lý prompt đó có còn quan trọng hay không? Attention U-Net khi không có hộp trả lời một câu hỏi khác hẳn, và Dice thấp của nó, gần 0.34 trên FracAtlas, chỉ đo phần việc tìm ra tổn thương. Thiết kế được kiểm chứng ở mọi chỗ mà cùng một hộp được đưa cho một mô hình khác hoặc bị lược bỏ một thành phần: các baseline Attention U-Net khớp prompt, các mô hình nền tảng nhận prompt đã fine-tune, và thí nghiệm ablation. Ở tất cả, PGA-UNet giữ được một khoảng cách khiêm tốn trên BTXRD, rõ rệt hơn trên FracAtlas, và lớn nhất ở nhóm Bottom-Dice của Attention U-Net cũng như ở các tổn thương nhỏ nhất. Nếu mục đích của việc giữ prompt hoạt động xuyên suốt encoder và decoder là để một tín hiệu tổn thương mờ nhạt không bị mất đi, thì nó phải giúp nhiều nhất đúng ở nơi tín hiệu mờ nhất, và đó là chiều hướng của kết quả: khoảng cách so với baseline khớp prompt, các mức chênh ablation từng thành phần, và độ bền khi dịch prompt đều lớn nhất trên FracAtlas, trên tổn thương nhỏ nhất, và dưới prompt lệch tâm, rồi cùng thu hẹp về phía dễ. Sự nhất quán về chiều hướng đó qua nhiều phép đo độc lập mới là chỗ dựa chính cho thiết kế, hơn bất kỳ so sánh đơn lẻ nào.
...
Nghiên cứu này cũng có những giới hạn khá cụ thể. Ảnh đầu vào bị resize về một hình vuông cố định, và chính các kết quả theo độ phân giải cho thấy điều đó gây tốn độ chính xác thật sự trên các tổn thương nhỏ nhất. Prompt trong bài đều được vẽ từ nhãn có sẵn, không phải do bác sĩ X-quang trực tiếp vẽ, và protocol giả định người dùng đã tìm ra đúng vùng rồi mới vẽ một hộp phủ lên nó. Trường hợp hộp chỉ phủ một phần, hộp âm, hay hộp sai vùng thì bài này chưa chạm tới; một phép kiểm tra trực tiếp cho thấy riêng điểm $Q$ vẫn ở mức cao trên nhiều hộp vẽ vào vùng giải phẫu lành, do trọng số mô hình chỉ được học trên ảnh có tổn thương. Muốn $Q$ báo được cả việc không có tổn thương thì cần huấn luyện thêm với vùng lành, hoặc ghép $Q$ với một cổng phát hiện hiện diện tổn thương riêng. BTXRD và FracAtlas được huấn luyện tách biệt hoàn toàn, không có mô hình chung và không có bước test chéo dataset nào, còn việc tách biệt theo từng bệnh nhân thì không thể xác nhận được từ metadata công khai. Các lần chạy Monte Carlo chỉ cho thấy sự ổn định qua các lần chia, chứ không chứng minh được tính vượt trội. Ablation từng thành phần chỉ trên một tập chia, nên nó cho thấy chiều hướng tác động của từng thành phần chứ chưa chốt được các đóng góp nhỏ hơn; chạy đa tập chia là hướng phát triển tiếp. Cuối cùng, các lợi ích khi prompt bị nhiễu loạn nhất quán với ý đồ thiết kế nhưng chưa được kiểm chứng bằng phân tích trực tiếp ở mức đặc trưng, nên cơ chế được suy ra từ hành vi chứ chưa đo được bên trong mạng.

\section{Kết luận \enlabel{Conclusion}}
Bài báo này trình bày PGA-UNet, một mạng CNN nhẹ có định hướng bằng prompt, dùng cho phân đoạn tương tác tổn thương xương trên ảnh X-quang. Thiết kế của mô hình biến một hộp bao phủ tổn thương do người dùng cung cấp thành một prior không gian dày đặc, rồi tái sử dụng prior này qua PSG ở phía encoder và CAD ở phía decoder. Trên hai bộ dữ liệu BTXRD và FracAtlas, đánh giá dưới cả prompt bao phủ lẫn lệch tâm có kiểm soát, PGA-UNet đạt ngang hoặc vượt ba mô hình nền tảng nhận prompt đã fine-tune (SAM-Med2D, MedSAM, ScribblePrompt) cũng như các baseline quy ước khớp prompt khi nhận cùng một hộp, với khoảng cách rõ nhất dưới prompt lệch tâm, ở những nơi khâu định vị tự động thất bại và ở những tổn thương nhỏ nhất, trong khi số tham số của mô hình chỉ bằng khoảng 1/92 so với SAM-Med2D và ngang cỡ mô hình nhỏ nhất trong ba. Các thực nghiệm chia lặp lại cho thấy xu hướng này khá ổn định, và hệ thống còn kèm hai tín hiệu hỗ trợ không cần nhãn: một điểm tự đánh giá xếp hạng độ tin cậy của mask cho các hộp thực sự chứa tổn thương, và một danh sách ngắn vùng ứng viên, cả hai còn ở mức thăm dò.

Kết luận này chỉ đúng trong phạm vi protocol đã dùng: hộp mô phỏng quanh tổn thương có nhãn. Nó chưa nói được gì về: phát hiện tự do, hộp sai vùng hay phủ một phần, prompt tự tay bác sĩ vẽ, độ tin cậy đã hiệu chuẩn, khả năng tổng quát theo bệnh nhân, hay việc chuyển sang dataset khác. Trước mắt, chúng tôi sẽ mở rộng ablation và các so sánh baseline qua nhiều tập chia, mở rộng đánh giá sang nhiều bộ dữ liệu hơn và hướng tới một mô hình dùng chung thay vì huấn luyện riêng cho từng bộ dữ liệu như hiện tại, đồng thời so sánh thêm với các mô hình y tế nhận prompt gần đây hơn. Về lâu dài, chúng tôi muốn đưa phương pháp gần hơn với điều kiện thực tế: thay các hộp mô phỏng bằng những protocol prompt lỏng và thực tế hơn, bỏ bước resize ảnh về kích thước cố định, và tận dụng thêm thông tin lâm sàng đi kèm để cải thiện cả kết quả phân đoạn lẫn chất lượng vùng gợi ý. Cuối cùng, để giảm chi phí gán nhãn và tiến gần hơn tới một công cụ dùng được trong lâm sàng, hướng đi tiếp theo là học bán giám sát, tận dụng việc vẽ một hộp giới hạn thô nhanh hơn nhiều so với vẽ mask chuẩn theo từng pixel. Chúng tôi muốn khai thác phần lớn dữ liệu chỉ có nhãn hộp rẻ này, kết hợp với một lượng nhỏ mask chuẩn, thay vì đòi hỏi mask chi tiết cho mọi mẫu như hiện tại, cùng với việc biến điểm tự đánh giá thành một tín hiệu đã hiệu chuẩn, và huấn luyện thêm với vùng không tổn thương để điểm cũng phản ánh được sự vắng mặt tổn thương, để dùng cho việc gợi ý vùng nghi ngờ trong một quy trình xem lại lâm sàng thực sự.
