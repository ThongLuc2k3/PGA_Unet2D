...
\item Mô tả hiệu ứng của các thành phần điều kiện hóa, phản hồi quan sát được với một độ dịch prompt định trước, độ biến thiên qua bốn tập chia mức ảnh lặp lại và bốn seed huấn luyện cho riêng PGA-UNet, và chi phí tính toán trong môi trường đã kiểm thử.
\end{itemize}
...
Một hướng tự động khác kết hợp trực tiếp phát hiện với phân đoạn. Cả hai bộ dữ liệu nguồn dùng trong bài đều báo cáo một baseline như vậy trong phần thẩm định kỹ thuật (technical validation) của chính bài công bố dataset: BTXRD đánh giá YOLOv8s-seg cho phát hiện và phân đoạn instance tổn thương tự động, còn FracAtlas đánh giá cùng kiến trúc này cho vùng gãy xương. YOLOv8s-seg phải tự định vị tổn thương, được chấm theo metric kiểu detection riêng của từng bài dataset gốc, và không được huấn luyện lại hay đánh giá lại trong bài báo này; chúng tôi trích dẫn nó như bối cảnh tài liệu cho cách hai bộ dữ liệu nguồn từng được dùng cho phân đoạn tự động, không phải một baseline định lượng có thể so trực tiếp theo protocol nhận box-prompt dùng trong phần còn lại của bài (Bảng~\ref{tab:relwork}).

Song song đó, phân đoạn y tế tương tác cũng phát triển, xuất phát từ quan sát rằng chỉ cần một chút hướng dẫn từ người dùng cũng đủ giúp bài toán đơn giản đi đáng kể. Các phương pháp đầu tiên dùng click, vẽ nguệch ngoạc (scribble), hoặc gợi ý theo đường trắc địa (geodesic); về sau, bounding box được bổ sung như một dạng prompt nhanh và thô. Gần đây hơn, các mô hình tương tác như ScribblePrompt có thể xử lý nhiều loại prompt trong cùng một khung, trên nhiều bộ dữ liệu y sinh, và các mô hình phân đoạn tương tác dựa trên transformer như SimpleClick mở rộng hướng này sang các benchmark ảnh tự nhiên với backbone vision-transformer đơn giản. Nhìn chung, các phương pháp này chuyển bớt gánh nặng khu trú từ mô hình sang người dùng. Với tổn thương X-quang rất nhỏ, sự chuyển giao đó đặc biệt có giá trị, vì bác sĩ thường chỉ ra được vùng nghi ngờ một cách thô ngay cả khi hệ thống tự động khó tìm ra nó đáng tin cậy trong toàn ảnh.

Các mô hình nền tảng nhận prompt đưa ý tưởng này đi xa hơn nữa. SAM mở đầu bằng cách đưa phân đoạn theo prompt trở thành bài toán tổng quát; SAM-Med2D và MedSAM đưa nó vào lĩnh vực y tế; các biến thể dùng adapter như EMedSAM và Med-SA giúp giảm chi phí khi chuyên biệt hóa các mô hình này cho dữ liệu y tế. Những mô hình đó khá linh hoạt, nhưng lại tiêu thụ prompt chủ yếu ở mask decoder, và mang theo một backbone rất lớn so với mạng nơ-ron tích chập (CNN) gọn nhẹ được khảo sát ở đây.

Việc điều kiện hóa đặc trưng trung gian theo một tín hiệu bên ngoài cũng được nghiên cứu ngoài phạm vi phân đoạn. FiLM áp dụng một phép biến đổi affine theo từng kênh, học được, lên bản đồ đặc trưng dựa trên một tín hiệu điều kiện. SPADE điều kiện hóa các lớp chuẩn hóa trung gian theo một bố cục ngữ nghĩa không gian, dùng cho tổng hợp ảnh. Cổng nhân theo kênh của PSG và số hạng cộng ở mức decoder của CAD gần với họ điều kiện hóa đặc trưng tổng quát này hơn là với các baseline nối kênh đầu vào được đánh giá trong bài; chúng tôi không nhận điều kiện hóa theo đặc trưng là một nguyên lý mới của riêng mình, mà nghiên cứu việc áp dụng và hiệu ứng thực nghiệm của nó khi dùng cho phân đoạn CNN có box-prompt ở nhiều tầng encoder và decoder cùng lúc.
...
\midrule
YOLOv8s-seg & Không có (detect+seg gộp) & Detector-segmenter một tầng; baseline tự động của chính bài BTXRD/FracAtlas, không train lại ở đây \\
\midrule
...
\midrule
SimpleClick & Click & Backbone vision-transformer đơn giản cho phân đoạn tương tác \\
\midrule
...
Chỉ dấu thứ hai, $S_{\mathrm{stab}}$, đo độ ổn định của dự đoán khi prompt bị nhiễu loạn nhẹ. Hộp được nhích ngẫu nhiên $K$ lần, mỗi lần với một độ dịch và co giãn nhỏ; mô hình được chạy lại cho từng hộp đã nhích, và $S_{\mathrm{stab}}$ chính là độ giao trên hợp (Intersection-over-Union, IoU) trung bình giữa mask nhị phân gốc $\hat{\mathbf{M}}$ và các mask thu được từ những lần nhích đó, $\hat{\mathbf{M}}_k$:
\begin{equation}
...
\midrule
BTXRD & 1.493 & 187 & 187 & 1.847 & 239 & 232 \\
...
\begin{table}[H]
\caption{PGA-UNet trên tập con ảnh có diện tích tổn thương thấp cố định gồm 50 ảnh mỗi bộ dữ liệu, qua các độ phân giải. Dice, IoU và CBL báo cáo dạng bao phủ / lệch tâm; mỗi cấu hình được chấm trong khung riêng của nó. Cặp HD95 là pixel gốc / khoảng cách chuẩn hóa dưới điều kiện bao phủ. $\Delta$Dice là chênh lệch prompt-bao-phủ so với cấu hình $512$.}
\label{tab:small-res}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
Trên tập con ảnh có diện tích tổn thương thấp này, khoảng cách của PGA-UNet so với baseline không-prompt vẫn lớn trên cả hai bộ dữ liệu, và hiệu ứng độ phân giải lớn hơn so với toàn tập test trên BTXRD; trên FracAtlas, toàn tập test vốn đã mất nhiều vì độ phân giải, nên nhóm phụ này không cho thấy thêm mức phạt độ phân giải nào nữa. Như đã nêu ở trên, tập con này là một lựa chọn mang tính mô tả và tự nó không xác lập được rằng phương pháp ưu tiên có lợi cho tổn thương nhỏ nói chung.

\subsection{Độ nhạy với dịch prompt}
\label{sec:robust}
Dice của PGA-UNet giảm 0.008 trên BTXRD và 0.014 trên FracAtlas dưới quy tắc lệch tâm định trước (Bảng~\ref{tab:robust}); trên tập con ảnh có diện tích tổn thương thấp gồm 50 ảnh của Mục~\ref{sec:small}, mức giảm vẫn dưới 0.01 ở cả hai bộ dữ liệu. Baseline kênh nhị phân có mức giảm nhỏ hơn trên FracAtlas (0.005), nên các kết quả không xác lập được rằng PGA-UNet luôn là mô hình ít nhạy cảm nhất với quy tắc này; baseline crop giảm nhiều hơn trên FracAtlas (0.027) và giảm tương đương trên BTXRD (0.008). PGA-UNet vẫn giữ Dice quan sát được cao nhất trong ba cấu hình của bảng này dưới cả hai điều kiện prompt trên cả hai bộ dữ liệu. Các quan sát này áp dụng cho quy tắc hộp bao phủ tổn thương đã đánh giá, không mô tả prompt do người dùng tự vẽ tùy ý, phủ một phần, hay sai vùng.
...
Để tham khảo, Attention U-Net không prompt về nguyên tắc không đổi với việc dịch hộp này (0.517 trên BTXRD, 0.342 trên FracAtlas ở cả hai điều kiện), còn ba mô hình tham chiếu nhận prompt đã fine-tune, mỗi mô hình chấm trong khung gốc riêng, mất từ 0.03 đến 0.08 Dice dưới cùng cấu trúc lệch tâm (Bảng~\ref{tab:robust-refs}); ba mô hình này không đưa vào Bảng~\ref{tab:robust} vì khung chấm điểm của chúng khác khung $512\times512$ chung dùng ở đó.

\begin{table}[H]
\caption{Độ nhạy quan sát được của ba mô hình tham chiếu nhận prompt đã fine-tune với cùng cấu trúc lệch tâm ở Bảng~\ref{tab:robust}, mỗi mô hình chấm trong khung gốc riêng (z: bao phủ, s: lệch tâm). Không đưa vào Bảng~\ref{tab:robust} vì các khung này khác khung $512\times512$ chung dùng ở đó.}
\label{tab:robust-refs}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{llccccc}
...
\subsection{Phân tích ổn định theo seed huấn luyện \enlabel{Training-Seed Stability Analysis}}
\label{sec:seedstability}
Monte Carlo cross-validation ở trên thay đổi tập chia mức ảnh trong khi giữ nguyên seed huấn luyện ở 22120196. Để đặc trưng hóa nguồn biến thiên bổ sung, tức chính sự ngẫu nhiên của lượt huấn luyện, chúng tôi đã huấn luyện riêng PGA-UNet ở $512\times512$ trên đúng tập chia cố định gốc (Bảng~\ref{tab:splits}), dùng thêm ba seed huấn luyện khác (19122003, 05052004, và 22120031), mọi cài đặt còn lại từ Mục~\ref{sec:training} giữ nguyên. Cả bốn lượt chạy dùng chung một tập chia train/validation/test cố định; chỉ seed chi phối khởi tạo trọng số, xáo trộn dữ liệu, và các hành vi ngẫu nhiên khác trong huấn luyện là thay đổi giữa chúng.

Phân tích này chỉ giới hạn ở PGA-UNet. Không baseline nào đã đánh giá, hai baseline prompt quy ước hay ba mô hình nền tảng nhận prompt, được huấn luyện lại qua nhiều seed, nên không có so sánh nào trong bài này khẳng định ưu thế ổn định qua các seed huấn luyện so với các baseline đó. Bảng~\ref{tab:seedstability} báo cáo trung bình $\pm$ độ lệch chuẩn mẫu qua đủ bốn seed (22120196, 19122003, 05052004, và 22120031).

\begin{table*}[t]
\caption{Ổn định theo seed huấn luyện của PGA-UNet ở $512\times512$ trên tập chia cố định gốc (Bảng~\ref{tab:splits}), bốn seed huấn luyện (22120196, 19122003, 05052004, và 22120031), trung bình $\pm$ độ lệch chuẩn mẫu ($\mathrm{ddof}=1$, $n=4$). Precision và Recall là giá trị chuẩn ở mức pixel cho mask đã merge mức ảnh, chỉ báo cáo trong bảng này; đây không phải Recall cấp tổn thương (lesion-level), thứ bài này không đo.}
\label{tab:seedstability}
\centering
\scriptsize
\begin{tabular}{llcccccc}
\toprule
Bộ dữ liệu & Prompt & Dice & IoU & Precision & Recall & HD95 & CBL \\
\midrule
\multirow{2}{*}{BTXRD}
& Bao phủ (\texttt{center\_zoom}) & $0.770\pm0.009$ & $0.646\pm0.013$ & $0.747\pm0.018$ & $0.828\pm0.017$ & $23.7\pm0.7$ & $0.915\pm0.003$ \\
& Lệch tâm (\texttt{center\_shift}) & $0.762\pm0.010$ & $0.639\pm0.013$ & $0.750\pm0.016$ & $0.817\pm0.013$ & $24.8\pm0.8$ & $0.907\pm0.004$ \\
\midrule
\multirow{2}{*}{FracAtlas}
& Bao phủ (\texttt{center\_zoom}) & $0.723\pm0.006$ & $0.582\pm0.005$ & $0.669\pm0.014$ & $0.834\pm0.018$ & $15.3\pm8.5$ & $0.905\pm0.017$ \\
& Lệch tâm (\texttt{center\_shift}) & $0.706\pm0.010$ & $0.565\pm0.010$ & $0.662\pm0.024$ & $0.807\pm0.007$ & $13.8\pm5.0$ & $0.892\pm0.009$ \\
\bottomrule
\end{tabular}
\end{table*}

Độ biến thiên theo seed huấn luyện phụ thuộc vào bộ dữ liệu và độ đo: độ biến thiên ở các độ đo chồng lấn lớn hơn trên BTXRD nhưng tương đương hoặc hơi nhỏ hơn trên FracAtlas, còn HD95 lại theo chiều ngược lại. Độ lệch chuẩn của Dice qua bốn seed huấn luyện là 0,009 đến 0,010 trên BTXRD và 0,006 đến 0,010 trên FracAtlas, so với độ biến thiên theo tập chia đã báo cáo ở Monte Carlo cross-validation (Bảng~\ref{tab:mccv}) là 0,004 đến 0,005 trên BTXRD và 0,007 đến 0,011 trên FracAtlas. IoU và CBL biến động ở mức tương đương Dice dưới cả hai nguồn biến thiên này. HD95 phá vỡ khuôn mẫu đó: trên BTXRD, độ lệch chuẩn của nó (0,7 đến 0,8 pixel) nhỏ hơn rõ rệt so với độ biến thiên theo tập chia Monte Carlo (2,0 đến 2,5 pixel), nhưng trên FracAtlas nó lại lớn hơn rõ rệt (5,0 đến 8,5 pixel ở đây so với 2,7 đến 3,9 pixel ở đó). Độ biến thiên HD95 trên FracAtlas do một seed ngoại lệ duy nhất gây ra, trong khi Dice, IoU và CBL của chính seed này vẫn bình thường (Bảng~\ref{tab:seedstability-perseed}); không seed nào trên BTXRD cho thấy một giá trị ngoại lệ tách biệt tương tự ở bất kỳ độ đo nào. Điều này cho thấy, ít nhất với FracAtlas, một độ đo khoảng cách biên có thể biến động qua các seed huấn luyện nhiều hơn hẳn so với các độ đo dựa trên chồng lấn, ngay cả khi tập chia và protocol huấn luyện được giữ cố định. Như xuyên suốt bài báo, không baseline nào được huấn luyện lại qua nhiều seed huấn luyện, nên phân tích này chỉ mô tả độ biến thiên của riêng PGA-UNet và không xác lập được độ ổn định so sánh với bất kỳ baseline nào.

\begin{table}[H]
\caption{Số liệu từng seed làm nền cho trung bình $\pm$ độ lệch chuẩn ở Bảng~\ref{tab:seedstability}, ở $512\times512$ trên tập chia cố định gốc, bốn seed huấn luyện (z: bao phủ, s: lệch tâm).}
\label{tab:seedstability-perseed}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{llcccc}
\toprule
Bộ dữ liệu & Seed & Dice z/s & IoU z/s & HD95 z/s & CBL z/s \\
\midrule
\multirow{4}{*}{BTXRD}
& 22120196 & 0.782 / 0.774 & 0.661 / 0.652 & 22.77 / 24.50 & 0.918 / 0.912 \\
& 19122003 & 0.760 / 0.750 & 0.629 / 0.622 & 24.51 / 25.15 & 0.911 / 0.902 \\
& 22120031 & 0.767 / 0.760 & 0.642 / 0.638 & 23.63 / 25.76 & 0.913 / 0.906 \\
& 05052004 & 0.773 / 0.766 & 0.650 / 0.643 & 23.87 / 23.79 & 0.917 / 0.910 \\
\midrule
\multirow{4}{*}{FracAtlas}
& 22120196 & 0.727 / 0.713 & 0.585 / 0.570 & 10.49 / 10.41 & 0.913 / 0.897 \\
& 19122003 & 0.728 / 0.712 & 0.587 / 0.571 & 10.67 / 11.55 & 0.905 / 0.900 \\
& 22120031 & 0.720 / 0.692 & 0.577 / 0.551 & 12.07 / 12.16 & 0.921 / 0.888 \\
& 05052004 & 0.716 / 0.708 & 0.578 / 0.568 & \textbf{28.07} / \textbf{21.24} & 0.882 / 0.881 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\caption{PGA-UNet đầy đủ trên cùng một ca BTXRD (IMG000054) với prompt Gaussian dạng cao nguyên so với prompt hộp nhị phân, cùng ở $512\times512$. Trong ca minh họa này, biến thể nhị phân mất khả năng định vị dù PSG và CAD không đổi.}
...
$Q$ (Spearman) & \best{0.70} & \best{0.60} & \best{0.65} & \best{0.48} \\
$Q$ (Pearson) & 0.82 & 0.76 & 0.59 & 0.44 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\textbf{Bộ lọc hậu kiểm thăm dò cùng MedSigLIP.} Chúng tôi thử xem một mô hình thị giác-ngôn ngữ có sẵn, riêng biệt, có ngăn được các hộp dương giả của PGA-UNet trên ảnh lành hay không, như một hướng thăm dò khác cho đúng phát hiện ở trên. Kiểm tra này chỉ dùng BTXRD, không dùng FracAtlas, và dùng lại đúng checkpoint PGA-UNet $512\times512$ đã huấn luyện như phần còn lại của bài. Chúng tôi chấm 50 hộp lấy mẫu mỗi ảnh bằng MedSigLIP, một bộ mã hóa ảnh-văn bản y khoa, dùng ở chế độ zero-shot như một bộ lọc hậu kiểm áp sau dự đoán của PGA-UNet, không phải một phần kiến trúc và không được fine-tune. Một tập hợp cố định các câu prompt dương/âm cho ra điểm cosine-margin. Ngưỡng cắt được chọn từ các hộp dương của tập validation để giữ lại ít nhất 95\% số đó; 410 ảnh lành của tập validation chỉ được dùng để đánh giá chẩn đoán, không dùng để chọn ngưỡng, và các ảnh test giữ lại (held-out) cũng không tham gia vào việc chọn ngưỡng này. Cohort hiệu chỉnh này (239 hộp dương của tập validation) lấy bằng cùng bộ tải polygon dùng xuyên suốt bài và khớp với 239 polygon validation của BTXRD ghi ở Bảng~\ref{tab:splits}. Tập test giữ lại là 1.469 ảnh còn lại trong cùng 1.879 ảnh lành BTXRD đã dùng ở trên, cho ra 73.450 hộp lấy mẫu.

Lọc bằng MedSigLIP hạ tỉ lệ dương giả cấp hộp trên các hộp giữ lại này từ 61,96\% xuống 44,62\%, mức giảm tuyệt đối 17,33 điểm phần trăm, và giữ lại 96,12\% số hộp dương trên chính các hộp tổn thương thuộc tập test BTXRD của bài (232 hộp từ 187 ảnh, dựng theo đúng quy tắc hộp bao phủ dùng xuyên suốt bài); cả hai thay đổi đều có khoảng tin cậy bootstrap theo cụm $95\%$ được báo cáo ở Bảng~\ref{tab:medsiglip-gate}. Tỉ lệ dương giả cấp ảnh, tức tỉ lệ ảnh lành còn ít nhất một hộp dương giả sống sót trong 50 hộp lấy mẫu, gần như không đổi, từ 100\% xuống 99,93\%: gần như mọi ảnh lành vẫn còn ít nhất một hộp qua mặt được hệ thống đã lọc. Chúng tôi không báo cáo Dice hậu-lọc hay độ đo chất lượng phân đoạn nào khác ở đây. Kết quả này mang tính thăm dò và bổ sung, không phải đóng góp cốt lõi của bài: một bộ lọc hậu kiểm ngoài có thể giảm phản hồi dương giả cấp hộp trong khi vẫn giữ được phần lớn hộp dương, nhưng chưa giải quyết được việc phát hiện dương giả cấp ảnh; phát triển một bộ lọc nhẹ, chuyên dụng, được hiệu chỉnh (calibration) tốt hơn là hướng phát triển tiếp theo (Mục~\ref{sec:discussion}).

\begin{table}[H]
\caption{Bộ lọc hậu kiểm zero-shot MedSigLIP trên ảnh lành BTXRD (tập test giữ lại, 1.469 ảnh, 50 hộp/ảnh, 73.450 hộp). Tỉ lệ giữ lại hộp dương được đo riêng trên các hộp tổn thương thuộc tập test BTXRD của bài (232 hộp, 187 ảnh). Cột CI là khoảng tin cậy bootstrap theo cụm $95\%$ cho mức giảm FPR cấp hộp và cho tỉ lệ giữ lại hộp dương, ở những hàng có báo cáo. Mang tính thăm dò, không phải đóng góp cốt lõi; không có Dice hậu-lọc.}
...
\begin{tabular}{lccc}
\toprule
Đại lượng & Trước lọc & Sau lọc & CI $95\%$ \\
\midrule
FPR cấp hộp (73.450 hộp) & 61,96\% & 44,62\% & $[16,68; 17,98]$ điểm giảm \\
FPR cấp ảnh (1.469 ảnh) & 100\% & 99,93\% & --- \\
Tỉ lệ giữ lại hộp dương (232 hộp) & 100,00\% & 96,12\% & $[93,42; 98,30]$ \\
\bottomrule
\end{tabular}
}
\end{table}
...
Các chênh lệch baseline chính là ước lượng điểm mang tính mô tả từ tập test cố định, mỗi cái là một so sánh fixed-run đơn lẻ chứ không phải lượt huấn luyện lặp lại. Thống kê ablation bắt cặp hiện có có điều kiện theo các checkpoint đã đánh giá, trong khi các tập chia bổ sung chỉ mô tả riêng PGA-UNet. Một phân tích ổn định theo seed huấn luyện giới hạn ở PGA-UNet (Mục~\ref{sec:seedstability}), qua đủ bốn seed huấn luyện, cho thấy mức biến thiên theo seed huấn luyện phụ thuộc vào bộ dữ liệu và độ đo so với biến thiên theo tập chia đã có: lớn hơn ở các độ đo chồng lấn trên BTXRD nhưng tương đương hoặc hơi nhỏ hơn trên FracAtlas, còn HD95 lại theo chiều ngược lại, kèm một seed FracAtlas có HD95 cao bất thường dù các độ đo chồng lấn vẫn bình thường; tự nó, phân tích này không xác lập được ưu thế ổn định so với bất kỳ baseline nào, vì không baseline nào được huấn luyện lại qua nhiều seed. Việc tách biệt theo từng bệnh nhân chưa được xác minh, và chênh lệch giữa số đa giác FracAtlas đã chuyển đổi với số trường hợp gốc vẫn chưa được đối chiếu ở mức annotation-ID. Hai bộ dữ liệu được huấn luyện với các mô hình riêng biệt, không có đánh giá chéo dataset. Do đó, nghiên cứu này ủng hộ một đánh giá thực nghiệm có giới hạn cho thiết kế đề xuất, chứ không phải những tuyên bố tổng quát về giá trị ứng dụng lâm sàng, ưu thế so sánh qua nhiều lượt huấn luyện, hay khả năng tổng quát chéo dataset.

\section{Kết luận \enlabel{Conclusion}}
Bài báo này trình bày PGA-UNet, một CNN gọn nhẹ đưa một prompt dạng hộp vào đặc trưng encoder và skip attention của decoder. Trên các tập test cố định BTXRD và FracAtlas, mô hình đạt Dice bao phủ 0,782 và 0,727 ở $512\times512$, với Dice quan sát được cao hơn các baseline prompt quy ước đã thử. So sánh dùng prompt nhị phân hiện có và các ablation theo thành phần cung cấp bằng chứng cho thiết kế điều kiện hóa đã đánh giá, với hiệu ứng phụ thuộc bộ dữ liệu. PGA-UNet có khoảng 2,96 triệu tham số, và các kết quả thời gian đã báo cáo mô tả chi phí tính toán của nó trong môi trường đã kiểm thử. Các phát hiện này giới hạn trong phạm vi prompt bao phủ tổn thương mô phỏng, đánh giá ở mức ảnh, và các cấu hình huấn luyện đã báo cáo. Đánh giá với prompt do bác sĩ tự vẽ vẫn là hướng phát triển tiếp theo. Một phân tích ổn định theo seed huấn luyện cho PGA-UNet (Mục~\ref{sec:seedstability}), qua bốn seed huấn luyện trên tập chia cố định gốc, cho thấy mức biến thiên theo seed huấn luyện phụ thuộc vào bộ dữ liệu và độ đo, lớn hơn ở các độ đo chồng lấn trên BTXRD nhưng tương đương hoặc hơi nhỏ hơn trên FracAtlas, còn HD95 lại theo chiều ngược lại; các lượt huấn luyện so sánh lặp lại cho các baseline đã đánh giá vẫn là hướng phát triển tiếp theo. Bộ lọc MedSigLIP mang tính thăm dò (Mục~\ref{sec:selfassess-results}) cho thấy một bộ lọc hậu kiểm ngoài có thể giảm dương giả cấp hộp trên ảnh lành trong khi vẫn giữ được phần lớn hộp dương, nhưng chưa giải quyết được việc phát hiện dương giả cấp ảnh; phát triển một bộ lọc nhẹ chuyên dụng, được hiệu chỉnh tốt hơn, cùng với một đánh giá chất lượng phân đoạn hậu-lọc, là hướng để lại cho phát triển tiếp theo.
...
OpenAI ChatGPT và Anthropic Claude được dùng để hỗ trợ rà soát bản thảo, biên tập ngôn ngữ, và soạn lại/cấu trúc lại một số đoạn trong Tóm tắt, Giới thiệu, Phương pháp, Thiết lập thực nghiệm, Kết quả, Thảo luận và Kết luận. Sự hỗ trợ này bao gồm việc chỉ ra các điểm chưa nhất quán và điều chỉnh câu chữ cũng như phạm vi của các tuyên bố, dựa trên phương pháp và kết quả do các tác giả báo cáo. Các tác giả chịu trách nhiệm về bản thảo cuối cùng.
...
\item \href{https://arxiv.org/abs/1709.07871}{Perez, Strub, de Vries, Dumoulin, Courville, ``FiLM: Visual Reasoning with a General Conditioning Layer,'' AAAI 2018.}
\item \href{https://openaccess.thecvf.com/content_CVPR_2019/html/Park_Semantic_Image_Synthesis_With_Spatially-Adaptive_Normalization_CVPR_2019_paper.html}{Park, Liu, Wang, Zhu, ``Semantic Image Synthesis With Spatially-Adaptive Normalization,'' CVPR 2019.}
\item \href{https://openaccess.thecvf.com/content/ICCV2023/html/Liu_SimpleClick_Interactive_Image_Segmentation_with_Simple_Vision_Transformers_ICCV_2023_paper.html}{Liu, Xu, Bertasius, Niethammer, ``SimpleClick: Interactive Image Segmentation with Simple Vision Transformers,'' ICCV 2023.}
\item \href{https://arxiv.org/abs/2507.05201}{Sellergren \textit{et al.}, ``MedGemma Technical Report,'' arXiv:2507.05201, 2025.}
\end{enumerate}
...
\textbf{Lý Quốc Ngọc} (Phó Giáo sư, Tiến sĩ) là Trưởng Bộ môn Thị giác Máy tính và Điều khiển học Nhận thức, Khoa Công nghệ Thông tin, Trường Đại học Khoa học Tự nhiên, ĐHQG-HCM. Ông nhận bằng Cử nhân ngành Cơ học và Toán học, bằng Thạc sĩ ngành Khoa học Máy tính, và bằng Tiến sĩ Khoa học ngành Khoa học Máy tính, đều tại Trường Đại học Khoa học Tự nhiên, TP. Hồ Chí Minh, lần lượt vào các năm 1987, 1996, và 2009. Hướng nghiên cứu: trí tuệ nhân tạo, học máy, thị giác máy tính, xử lý ảnh và video số, đồ họa máy tính, phân tích thống kê đa biến, và phân tích ảnh y tế. Ông là tác giả hoặc đồng tác giả của gần 70 bài báo tại các hội nghị và tạp chí khoa học, và từng là chủ nhiệm của nhiều đề tài nghiên cứu về hệ thống thị giác thông minh.
