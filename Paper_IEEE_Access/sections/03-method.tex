...
During evaluation, prompts are tested in two settings. A covering prompt scales the tight lesion box outward from its center by a fixed factor of 3.0. An off-center prompt first applies the same scaling and then adds a displacement controlled by \texttt{shift\_ratio=0.5}, while preserving lesion coverage. A single model is trained per dataset and resolution using \texttt{center\_mixed}, which selects \texttt{center\_shift} with probability 0.8 and \texttt{center\_zoom} with probability 0.2. The two conditions are evaluated separately at test time. The task scope assumes that the user has identified the suspicious region and provides a box intended to cover that lesion; partial-coverage, negative, and unrelated-region boxes are outside this controlled prompted-segmentation protocol.

Let $(x_1,y_1,x_2,y_2)$ denote the tight lesion box and let $w=x_2-x_1$ and $h=y_2-y_1$. The simulated covering box uses the fixed center scaling described above during training and evaluation. The off-center box uses the same scaled box and applies the deterministic test-time displacement rule implemented in \texttt{dataset.py}, while preserving lesion coverage. These values were selected through preliminary protocol experiments and then fixed before the reported evaluation; they are not presented as globally optimal hyperparameters. This simulation is not a substitute for radiologist-drawn prompts, but it provides a controlled way to measure sensitivity to the defined prompt displacement.
