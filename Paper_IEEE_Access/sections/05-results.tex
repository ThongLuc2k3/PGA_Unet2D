...
\begin{table}[t]
\caption{PGA-UNet on the fixed 50-image lower-area subset of each dataset across resolutions. Dice, IoU, and CBL are reported as covering / off-center; each configuration is scored in its own frame. The HD95 pair denotes native pixels / normalized distance under the covering condition. $\Delta$Dice is the covering-prompt difference relative to the $512$ configuration.}
\label{tab:small-res}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
\begin{table}[t]
\caption{Paired comparison of the full model against each ablated variant, per-image Dice on the single split. $\Delta$ is the mean per-image Dice difference (full minus variant), tested with a two-sided paired sign-flip randomization test ($20{,}000$ permutations). $^{**}$ marks entries that stay significant ($p<0.05$) after Holm correction over the full family of ablation tests (four ablated model contrasts $\times$ two datasets $\times$ two prompt conditions, all on Dice, the only metric tested here); $^{*}$ marks raw $p<0.05$ that does not survive correction.}
