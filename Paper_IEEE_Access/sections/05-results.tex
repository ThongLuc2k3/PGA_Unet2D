...
\label{sec:sam}
Three promptable foundation models receive the same box: SAM-Med2D (brought into medicine by broad fine-tuning), MedSAM (medical fine-tuning, box-only), and ScribblePrompt-UNet (a lightweight UNet, not a SAM-style architecture). In zero-shot mode, all three are close to unusable on bone X-ray images: 0.25 to 0.43 Dice, with very low precision (0.16 to 0.34) because the mask spills outside the lesion. Fine-tuning on the same split and the same prompt protocol adds 0.24 to 0.42 Dice (Table~\ref{tab:sam}). After fine-tuning, on BTXRD PGA-UNet at $512$ (0.782) leads all three (MedSAM 0.724, ScribblePrompt 0.653, SAM-Med2D 0.630). On FracAtlas under covering prompts, fine-tuned MedSAM (0.729) matches PGA-UNet-512 (0.727); but under off-center prompts PGA-UNet (0.713) pulls well ahead of MedSAM (0.666), and PGA-UNet leads on CBL in every cell and on HD95 in most, while using about $1/90$ of MedSAM's parameters and a smaller input. ScribblePrompt-UNet, despite a parameter count close to PGA-UNet's, trails by 0.05 to 0.13 Dice and has the largest covering-to-off-center drop in the group. Each foundation model is evaluated at its own fixed input resolution, so this comparison is not perfectly resolution-matched. The change from covering to off-center prompts is discussed separately in Section~\ref{sec:robust}.

\begin{table}[t]
\caption{Comparison against promptable baselines, image-level merged, covering prompts. Each foundation model runs at its own fixed input resolution; PGA-UNet is shown at $512$.}
\label{tab:sam}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
\begin{table}[t]
\caption{Sensitivity to prompt displacement. Every model under covering (z) and off-center (s) prompts. Attention U-Net (no prompt) is prompt-invariant. AttUNet rows and PGA-UNet at $512\times512$; the promptable foundation models at their own native resolutions (SAM-Med2D $256$, MedSAM $1024$, ScribblePrompt $128$), image-level merged.}
\label{tab:robust}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
