...
\label{sec:extreme}
We ranked the test images by Attention U-Net Dice, kept the top 50 and bottom 50 per dataset, and re-ran all four models on those exact images, to probe the same comparison on a different split (Table~\ref{tab:extreme}, covering prompts). On the BTXRD top-50 subset, the three prompted configurations have Dice values between 0.844 and 0.862. On the FracAtlas top-50 subset, their values span a wider range, from 0.605 to 0.754. On the bottom-50, Attention U-Net's Dice is far lower, down to 0.031 on BTXRD and 0.169 on FracAtlas, while PGA-UNet's is 0.712 and 0.687, and PGA-UNet leads the two conventional prompt baselines by 0.04 to 0.07 Dice on BTXRD and by 0.10 to 0.19 on FracAtlas within this group.
...
For reference, the no-prompt Attention U-Net is by construction unaffected by this box displacement (0.517 on BTXRD, 0.342 on FracAtlas under both conditions), and the three fine-tuned promptable reference models, each scored in its own native frame, lose 0.03 to 0.08 Dice under the same off-center construction (Table~\ref{tab:robust-refs}); these three are not included in Table~\ref{tab:robust} because their scoring frames differ from the shared $512\times512$ frame used there.

\begin{table}[t]
\caption{Observed sensitivity of the three fine-tuned promptable reference models to the same off-center construction as Table~\ref{tab:robust}, each scored in its own native frame (z: covering, s: off-center). Not included in Table~\ref{tab:robust} because these frames differ from the shared $512\times512$ frame used there.}
...
\caption{Full PGA-UNet on the same BTXRD case (IMG000054) with a Gaussian-plateau prompt vs a binary box prompt, both at $512\times512$. In this illustrative case, the binary variant loses localization even though PSG and CAD are unchanged.}
...
The Monte Carlo cross-validation above varies the image-level split while keeping the training seed fixed at 22120196. To characterize the complementary source of variability, training-run randomness itself, we separately trained PGA-UNet at $512\times512$ on the original fixed split (Table~\ref{tab:splits}) using three additional training seeds (19122003, 05052004, and 22120031), with every other setting from Section~\ref{sec:training} held fixed. All four runs use the same fixed train/validation/test partition; only the seed governing weight initialization, data shuffling, and other stochastic training behavior changes between them.

This analysis is restricted to PGA-UNet. None of the evaluated baselines, the two conventional prompt baselines or the three promptable foundation models, has been retrained across multiple seeds, so no comparison in this article claims stable superiority over those baselines across training seeds. Table~\ref{tab:seedstability} reports the mean $\pm$ sample standard deviation over all four seeds (22120196, 19122003, 05052004, and 22120031).

\begin{table*}[t]
\caption{Training-seed stability of PGA-UNet at $512\times512$ on the original fixed split (Table~\ref{tab:splits}), four training seeds (22120196, 19122003, 05052004, and 22120031), mean $\pm$ sample standard deviation ($\mathrm{ddof}=1$, $n=4$). Precision and recall are the standard pixel-level values for the image-level merged mask, reported only in this table; they are not a lesion-level detection recall, which this article does not measure.}
...
\midrule
\multirow{2}{*}{BTXRD}
& Covering (\texttt{center\_zoom}) & $0.770\pm0.009$ & $0.646\pm0.013$ & $0.747\pm0.018$ & $0.828\pm0.017$ & $23.7\pm0.7$ & $0.915\pm0.003$ \\
& Off-center (\texttt{center\_shift}) & $0.762\pm0.010$ & $0.639\pm0.013$ & $0.750\pm0.016$ & $0.817\pm0.013$ & $24.8\pm0.8$ & $0.907\pm0.004$ \\
\midrule
\multirow{2}{*}{FracAtlas}
& Covering (\texttt{center\_zoom}) & $0.723\pm0.006$ & $0.582\pm0.005$ & $0.669\pm0.014$ & $0.834\pm0.018$ & $15.3\pm8.5$ & $0.905\pm0.017$ \\
& Off-center (\texttt{center\_shift}) & $0.706\pm0.010$ & $0.565\pm0.010$ & $0.662\pm0.024$ & $0.807\pm0.007$ & $13.8\pm5.0$ & $0.892\pm0.009$ \\
\bottomrule
\end{tabular}
\end{table*}

Training-seed variability was dataset- and metric-dependent: overlap-metric variability was larger on BTXRD but comparable to or slightly smaller on FracAtlas, whereas HD95 showed the opposite pattern. Dice's standard deviation across the four training seeds is 0.009 to 0.010 on BTXRD and 0.006 to 0.010 on FracAtlas, versus a split-seed spread of 0.004 to 0.005 on BTXRD and 0.007 to 0.011 on FracAtlas already reported for Monte Carlo cross-validation (Table~\ref{tab:mccv}). IoU and CBL vary by a similar magnitude to Dice under both sources of variation. HD95 breaks this pattern: on BTXRD its standard deviation (0.7 to 0.8 pixels) is markedly smaller than the Monte Carlo split-seed spread (2.0 to 2.5 pixels), but on FracAtlas it is markedly larger (5.0 to 8.5 pixels here versus 2.7 to 3.9 pixels there). The FracAtlas HD95 spread is driven by a single outlier seed whose Dice, IoU, and CBL remain unremarkable (Table~\ref{tab:seedstability-perseed}); no BTXRD seed shows a comparably isolated outlier on any metric. This indicates that, at least for FracAtlas, a boundary-distance metric can vary far more across training seeds than the overlap-based metrics do, even holding the split and training protocol fixed. As throughout this article, no baseline has been retrained across multiple training seeds, so this analysis characterizes PGA-UNet's own variation only and does not establish comparative stability against any baseline.

\begin{table}[t]
\caption{Per-seed results underlying Table~\ref{tab:seedstability}'s mean $\pm$ SD, at $512\times512$ on the original fixed split, four training seeds (z: covering, s: off-center).}
...
\begin{tabular}{llcccc}
\toprule
Dataset & Seed & Dice z/s & IoU z/s & HD95 z/s & CBL z/s \\
\midrule
\multirow{4}{*}{BTXRD}
...
\label{sec:loss}
The segmentation targets in this article are small, so we tried replacing the Dice term with a size-conditioned Tversky loss, and in another trial, with a Focal Dice loss, with every other setting held fixed (Table~\ref{tab:loss}). Neither option worked better on Dice, which is best or tied for the default Dice + BCE objective on both datasets and both prompt conditions; HD95 follows the same pattern. CBL mostly favors the default too, with one exception: on FracAtlas covering, Tversky's CBL (0.918) is slightly higher than the default's (0.913), even though its Dice, IoU, and HD95 are all worse there. Focal Dice is a clear step backward on FracAtlas on every metric, with HD95 exceeding 100 pixels. We treat this as a negative control: under the current protocol, the default objective remains the better choice on the metric it is trained to optimize, and neither alternative is adopted.
...
$Q$ (Spearman) & \textbf{0.70} & \textbf{0.60} & \textbf{0.65} & \textbf{0.48} \\
...
\paragraph{Exploratory post-hoc gate with MedSigLIP}
We tested whether a separate, off-the-shelf vision-language model could suppress PGA-UNet false-positive boxes on lesion-free images, as one exploratory response to the finding above. This check uses BTXRD only, not FracAtlas, and reuses the same trained PGA-UNet $512\times512$ checkpoint as the rest of this article. We scored 50 sampled boxes per image with MedSigLIP \cite{b18}, a medical image-text encoder, used zero-shot as a post-hoc gate applied after PGA-UNet's prediction, not as part of the architecture and not fine-tuned. A fixed ensemble of positive and negative text prompts yields a cosine-margin score. The cutoff was selected from the positive validation boxes to retain at least 95\% of them; the 410 lesion-free validation images were used only for diagnostic evaluation, not for threshold selection, and the held-out test images play no part in choosing this threshold either. This calibration cohort (239 positive validation boxes) is drawn with the same polygon loader used throughout this article and matches the 239 BTXRD validation polygons reported in Table~\ref{tab:splits}. The held-out test split is the remaining 1{,}469 of the same 1{,}879 lesion-free BTXRD images used above, giving 73{,}450 sampled boxes.

Gating with MedSigLIP lowers the box-level false-positive rate on these held-out boxes from 61.96\% to 44.62\%, an absolute reduction of 17.33 points, and retains 96.12\% of positive boxes on the article's own BTXRD test-split lesion boxes (232 boxes from 187 images, built with the same covering-box construction used throughout this article); both changes have cluster-bootstrap $95\%$ confidence intervals reported in Table~\ref{tab:medsiglip-gate}. Image-level false-positive rate, the fraction of lesion-free images with at least one surviving false-positive box among the 50 sampled, barely moves, from 100\% to 99.93\%: nearly every lesion-free image still has at least one box that fools the gated system. We do not report a post-gate Dice or other segmentation-quality measure here. This result is exploratory and supplementary, not a core contribution of this article: an external post-hoc gate can reduce box-level false responses while keeping most positive boxes, without solving image-level false-positive detection; developing a purpose-built, better-calibrated lightweight gate is future work (Section~\ref{sec:discussion}).

\begin{table}[t]
\caption{MedSigLIP zero-shot post-hoc gate on BTXRD lesion-free images (held-out test split, 1{,}469 images, 50 boxes/image, 73{,}450 boxes). Positive-box retention is measured separately on the article's own BTXRD test-split lesion boxes (232 boxes, 187 images). The CI column reports the cluster-bootstrap $95\%$ interval for the box-FPR reduction and for the retained-positive-box rate, where computed. Exploratory, not a core contribution; no post-gate Dice is reported.}
...
\begin{tabular}{lccc}
\toprule
Quantity & Before gate & After gate & $95\%$ CI \\
\midrule
Box FPR (73{,}450 boxes) & 61.96\% & 44.62\% & $[16.68, 17.98]$ pt reduction \\
Image FPR (1{,}469 images) & 100\% & 99.93\% & --- \\
Positive-box retention (232 boxes) & 100.00\% & 96.12\% & $[93.42, 98.30]$ \\
\bottomrule
\end{tabular}
}
\end{table}
