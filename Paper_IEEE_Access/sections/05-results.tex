...
With only two completed seeds, these standard deviations are not yet informative about the true seed-to-seed spread and should not be read as a stable estimate; they describe only the two runs observed so far. Read as a two-point comparison, not a distribution: the newly completed seed's Dice is about 0.02 lower than the original seed's on BTXRD under both prompt conditions, while on FracAtlas the two seeds are within 0.002 Dice of each other. Precision differs by 0.024 to 0.034 between the two BTXRD seeds, more than the 0.000 to 0.002 difference in Recall there, and more than either metric's spread on FracAtlas (0.007 to 0.011). HD95 differs by about 1 to 2 pixels on BTXRD covering and by about 1 pixel on FracAtlas off-center, and by under 1 pixel on the other two rows. Whether this pattern, larger BTXRD sensitivity, holds up is exactly what the two remaining seeds will show; nothing here yet supports a general claim about which dataset or metric is more sensitive to the training seed.
