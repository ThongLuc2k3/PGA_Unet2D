...
- SAM-Med2D finetuning uses the PGA covering-prompt protocol for training too (50/50 per sample between zoom-out expansion, ratio 0.15-0.45, and off-center shift, ratio 0.30, via `prompt_box_from_mask` in `DataLoader.py`), not the original authors' `get_boxes_from_mask` box-noise: training SAM-Med2D only on the authors' tight-box jitter while testing it under PGA's wider zoom-out/shift prompts would confound the comparison, since PGA-UNet trains on the exact distribution it is tested on. `train.py`'s `TRAIN_PROMPT_MODE` now defaults to `'mixed'` (`dataset.py`'s `prompt_mode='mixed'`: an independent 50/50 per-sample pick between `zoom_out` and `shift`), so PGA-UNet and SAM-Med2D are both trained on the same symmetric covering/off-center mixture; `'zoom_out'`-only and `'shift'`-only remain available as optional single-condition ablation modes. Validation during training and the test split use the same covering-prompt protocol as well, so training and evaluation are apples-to-apples across baselines. Training is also box-only: the point-prompt branch and the `iter_point` point-refinement loop are disabled (`iter_point=0`), unlike the original authors' training which mixes box and point prompts.
- **Pending retrain (as of 2026-08-19):** the `mixed`-mode training code above is new; every PGA-UNet checkpoint behind the tracked `Results/` notebooks and the paper's numeric tables (2, 4, 5, 6, 7, 9, 10) was still produced by the older zoom-out-only training code. The paper's prose was updated ahead of retraining, at the author's explicit request, to describe the new mixed-prompt protocol as the intended methodology going forward. Until real retraining happens and the tables are refreshed, the numeric tables reflect the old zoom-out-only checkpoints while the surrounding prose describes the new mixed protocol; do not assume the tracked numbers already reflect mixed-prompt training.
